...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        // --- 5. Remote Content Initialisation ---
        /**
        ...
        const initRemoteContent = async function() {
            if (!remoteConfig || !remoteConfig.changelogUrl) {
                ...
                if (_changelog.length > 0) {
                    loggerInterface.debug('remote', `Changelog source: static only (${_changelog.length} entries, no remoteConfig provided)`);
                } else {
                    loggerInterface.debug('remote', 'Changelog source: none (no static entries, no remoteConfig provided)');
                }
                return;
            }
            const {
            ...
            try {
                ...
                loggerInterface.debug('remote', `Changelog source: remote — replaced ${localCount} local entr${localCount === 1 ? 'y' : 'ies'} with ${_changelog.length} remote entries`);
