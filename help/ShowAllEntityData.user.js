...
// CHANGELOG
let changelog = [
    {version: '9.39.0+2026-02-19', description: 'Enhancement: (1) "Page Reloaded" popup now auto-triggers the corresponding "Show all…" action button after a 2-second countdown displayed on the OK button — clicking OK or pressing Enter immediately triggers the button without waiting. Escape still cancels the auto-action. (2) Added ❓ application help button (always visible, right of ⚙️): opens a scrollable popup dialog presenting a full feature overview of the script, closeable via "Close" button or Escape key.' },
...
(function() {
     ...
     */
    /**
     * "Page Reloaded" dialog with 2-second auto-action countdown.
     * After 2 seconds the dialog closes and the trigger button is auto-clicked.
     * Clicking OK (or pressing Enter) immediately performs the same action.
     * Pressing Escape cancels the auto-action and closes the dialog without clicking.
     *
     * @param {HTMLElement|null} actionButton - The "Show all…" button to auto-click
     */
    function showPageReloadedDialog(actionButton = null) {
        ...
        headerDiv.textContent = '⚠️ Page Reloaded';
        dialogDiv.appendChild(headerDiv);

        // Message
        const msgDiv = document.createElement('div');
        msgDiv.style.cssText = 'margin-bottom: 16px; line-height: 1.6; color: #555; font-size: 1.05em; word-wrap: break-word;';
        if (actionButton) {
            const btnLabel = actionButton.textContent.trim();
            msgDiv.textContent = `The underlying MusicBrainz page has been reloaded to ensure filter stability. The "${btnLabel}" button will be clicked automatically in a moment.`;
        } else {
            msgDiv.textContent = 'The underlying MusicBrainz page has been reloaded to ensure filter stability. Please click the desired "Show all" button again to start the process.';
        }
        dialogDiv.appendChild(msgDiv);

        // Countdown progress bar
        const progressTrack = document.createElement('div');
        progressTrack.style.cssText = 'height: 4px; background: #eee; border-radius: 2px; margin-bottom: 20px; overflow: hidden;';
        const progressFill = document.createElement('div');
        progressFill.style.cssText = 'height: 100%; background: #4CAF50; border-radius: 2px; transition: width 0.1s linear; width: 100%;';
        progressTrack.appendChild(progressFill);
        if (actionButton) {
            dialogDiv.appendChild(progressTrack);
        }
        ...
        // Cancel button (only shown when an action button exists)
        let cancelBtn = null;
        if (actionButton) {
            cancelBtn = document.createElement('button');
            cancelBtn.textContent = 'Cancel auto-action';
        ...
        // OK button (shows countdown when action button present)
        ...
        // Position below action button or center
        setTimeout(() => {
            if (actionButton) {
                const btnRect = actionButton.getBoundingClientRect();
                const dlgRect = dialogDiv.getBoundingClientRect();
        ...
        const performAction = () => {
            dismissDialog();
            if (actionButton) {
                Lib.info('fetch', 'Page Reloaded: auto-clicking "Show all" action button');
                actionButton.click();
            }
        };
        ...
        const cancelAction = () => {
            dismissDialog();
            Lib.info('fetch', 'Page Reloaded: auto-action cancelled by user');
        };

        // Wire OK button
        okBtn.onclick = performAction;
        ...
        // Start countdown only when action button is available
        if (actionButton) {
            okBtn.textContent = `OK (${(COUNTDOWN_MS / 1000).toFixed(1)}s)`;
            ...
            countdownInterval = setInterval(() => {
                ...
                okBtn.textContent = `OK (${(remaining / 1000).toFixed(1)}s)`;
        ...
        Lib.info('ui', 'Page Reloaded dialog shown' + (actionButton ? ' with auto-action countdown' : ''));
    }
    ...
    if (reloadFlag) {
        ...
        showPageReloadedDialog(firstActionBtn);
    }
