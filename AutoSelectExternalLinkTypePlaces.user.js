...
// @downloadURL  https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypePlaces.user.js
// @updateURL    https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypePlaces.user.js
...
(function() {
    ...
    const LOG_PREFIX = '[VZ:discography-autoselect-places]';

    function log(...args) {
        if (DEBUG) console.log(LOG_PREFIX, ...args);
    }

    const STORAGE_KEY_PLACES = 'MB_AutoSelect_Mappings_Places';

    const LINK_TYPES_OPTIONS_PLACES = [
    ...
    function loadMappings() {
        try {
            ...
            if (stored) {
                return JSON.parse(stored) || DEFAULT_MAPPINGS_PLACES;
            }
        } catch (e) {
            log("Failed to load mappings from localStorage. Using defaults.", e);
        }
        return DEFAULT_MAPPINGS_PLACES;
    }

    function saveMappings(mappings) {
        try {
            localStorage.setItem(STORAGE_KEY_PLACES, JSON.stringify(mappings));
...
//            const match = pastedText.match(regexSpringsteenClean);
                ...
                mutation.addedNodes.forEach(node => {
                    ...
                    if (selectElement && lastCleanedUrlData) {
                        ...
                        setTimeout(() => {
                            ...
                            if (linkValueToSet) {
                                const selectedOption = LINK_TYPES_OPTIONS_PLACES.find(o => o.id === linkValueToSet);
                                if (selectedOption) {
    ...
    function renderMappings(modalBody, openEditModalCallback) {
        ...
        linkMappings.forEach((mapping, index) => {
            const option = LINK_TYPES_OPTIONS_PLACES.find(opt => opt.id === mapping.typeId);
    ...
    function openEditModal(index, reEnableMainEscape) {
        ...
        const currentMapping = isEditing ? linkMappings[index] : { regex: '', typeId: LINK_TYPES_OPTIONS_PLACES[0].id, description: '' };
        ...
        modalContainer.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select id="typeSelect" style="padding: 5px; border: 1px solid #ccc; width: 100%;">
                    ${LINK_TYPES_OPTIONS_PLACES.map(opt =>
