...
// CHANGELOG
let changelog = [
    {version: '9.49.0+2026-02-19', description: 'Feature: Semantic group-stripe coloring for single-table pages. When any sort is active the background color of rows alternates in blocks: it only toggles when the value in the primary sort column (first in multiSortColumns, or lastSortIndex for single-column) changes, visually chunking rows into groups so secondary-sort structure is immediately visible. New applyGroupStripes(rows) helper called at the end of renderFinalTable; works for both fast and chunked rendering paths. Two new configSchema settings: sa_group_stripe_color_a (default #ffffff) and sa_group_stripe_color_b (default #ddeeff). Stripes are cleared automatically when no sort is active (restoring native MusicBrainz row backgrounds). Multi-table pages are unaffected.'},
...
(function() {
    ...
    const configSchema = {
        ...
        divider_group_stripe: {
            type: 'divider',
            label: '🦓 GROUP STRIPE COLORS'
        },

        sa_group_stripe_enabled: {
            label: 'Enable Group Stripe Coloring',
            type: 'checkbox',
            default: true,
            description: 'When a sort is active, alternate row background colors in blocks whenever the primary sort column value changes (semantic zebra striping)'
        },

        sa_group_stripe_color_a: {
            label: 'Group Stripe — Color A',
            type: 'color_picker',
            default: '#ffffff',
            description: 'Background color for even-numbered groups (first group, third group, …)'
        },

        sa_group_stripe_color_b: {
            label: 'Group Stripe — Color B',
            type: 'color_picker',
            default: '#ddeeff',
            description: 'Background color for odd-numbered groups (second group, fourth group, …)'
        },
    ...
    /**
     * Applies semantic group-stripe background coloring to a rendered row array.
     *
     * Color only toggles when the value in the PRIMARY sort column changes, visually
     * chunking contiguous groups so secondary-sort structure is immediately apparent.
     * Operates entirely on TR.style.backgroundColor — no class mutation, safe to call
     * repeatedly on every render pass.
     *
     * Primary column is derived from the current sort state of 'main_table':
     *   - Multi-column sort:  first entry of multiSortColumns
     *   - Single-column sort: lastSortIndex
     *   - No active sort:     clears all backgrounds and returns
     *
     * @param {HTMLTableRowElement[]} rows - The ordered array of TR elements to stripe.
     */
    function applyGroupStripes(rows) {
        if (!Lib.settings.sa_group_stripe_enabled) {
            // Feature disabled — ensure all rows are uncolored
            rows.forEach(r => { r.style.backgroundColor = ''; });
            return;
        }

        const state = multiTableSortStates.get('main_table');
        if (!state) return;

        // Determine the primary sort column index
        let primaryColIndex = -1;
        if (state.multiSortColumns && state.multiSortColumns.length > 0) {
            primaryColIndex = state.multiSortColumns[0].colIndex;
        } else if (state.lastSortIndex >= 0 && state.sortState !== 0) {
            primaryColIndex = state.lastSortIndex;
        }

        const colorA = Lib.settings.sa_group_stripe_color_a || '#ffffff';
        const colorB = Lib.settings.sa_group_stripe_color_b || '#ddeeff';

        if (primaryColIndex < 0) {
            // No sort active — clear stripes so native MusicBrainz backgrounds show through
            rows.forEach(r => { r.style.backgroundColor = ''; });
            return;
        }

        let band = 0; // 0 → colorA, 1 → colorB
        let prevValue = null;

        rows.forEach(row => {
            const cell = row.cells[primaryColIndex];
            const value = cell ? getCleanVisibleText(cell).trim().toLowerCase() : '';

            if (prevValue !== null && value !== prevValue) {
                band = 1 - band; // toggle band only on value change
            }
            prevValue = value;

            row.style.backgroundColor = band === 0 ? colorA : colorB;
        });

        Lib.debug('render', `applyGroupStripes: striped ${rows.length} rows on column ${primaryColIndex} (bands: ${band + 1})`);
    }
    ...
    async function renderFinalTable(rows) {
        ...
        // Apply semantic group-stripe coloring based on the active primary sort column
        applyGroupStripes(rows);
