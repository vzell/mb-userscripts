...
// CHANGELOG
let changelog = [
    {version: '9.55.0+2026-02-20', description: 'Fix: No highlighting at all in v9.54 — testRowMatch() referenced filterHighlightEnabled and prefilterHighlightEnabled via closure, but due to JavaScript scope/TDZ uncertainty between function definitions across thousands of lines, the flags were not reliably visible. Fixed by passing both flags explicitly through matchCtx (constructed in runFilter()) and destructuring them inside testRowMatch() as hlEnabled / preHlEnabled, eliminating any closure ambiguity entirely.'},
    {version: '9.54.0+2026-02-20', description: 'Fix: Definitive rewrite of highlight toggle logic (following Gemini\'s analysis). Root cause: runFilter() always works on clones from allRows/groupedRows, so any DOM manipulation of live highlight spans is immediately undone on the next runFilter() call. Fix: (1) testRowMatch() now guards highlightText() calls with filterHighlightEnabled, so clones never receive filter highlights when toggled off. (2) testRowMatch() also strips .mb-pre-filter-highlight spans from clones when prefilterHighlightEnabled is off — prefilter spans are baked into allRows at load time and would otherwise be inherited by every clone. (3) Both toggle button onclicks simplified to: flip flag → update appearance → call runFilter(). (4) Removed the four now-unnecessary save/restore functions (savePrefilterHighlightState, restorePrefilterHighlightState, saveFilterHighlightState, restoreFilterHighlightState) and their associated state variables (savedPrefilterHighlights, savedFilterHighlights).'},
    {version: '9.53.0+2026-02-20', description: 'Fix: Completely reworked the highlight toggle mechanism. Previous attempts (v9.51/v9.52) guarded highlightText() inside testRowMatch() with filterHighlightEnabled, which broke initial highlighting. New approach: (1) testRowMatch() is reverted to always apply highlights — keeping it simple. (2) A single gate at the end of runFilter() strips all filter-highlight spans when filterHighlightEnabled is false, after the rows are rendered. (3) Toggle onclick simplified: turn-off sets flag then strips DOM spans; turn-on sets flag then calls runFilter() directly, which re-applies highlights that the gate now keeps. The save/restore indirection (saveFilterHighlightState / restoreFilterHighlightState) is no longer used for the toggle button.'},
    {version: '9.52.0+2026-02-20', description: 'Fix: After toggling highlighting OFF and then ON again, nothing was highlighted. Cause: restoreFilterHighlightState() internally calls runFilter(), which now correctly checks filterHighlightEnabled — but the flag was still false at that point because the onclick set it to true only after the restore call. Fixed by setting filterHighlightEnabled = true before calling restoreFilterHighlightState().'},
    {version: '9.51.0+2026-02-20', description: 'Fix: "Toggle highlighting" button was completely ineffective — typing in any filter input called runFilter() → testRowMatch(), which unconditionally called highlightText() without ever checking filterHighlightEnabled. Highlighting was therefore re-applied on every keystroke regardless of toggle state. Fixed by guarding both highlightText() calls inside testRowMatch() with if (filterHighlightEnabled).'},
    {version: '9.50.0+2026-02-20', description: 'Fix: "Toggle highlighting" button visibility now correctly tracks whether any filter is active (global or column), not whether highlight spans happen to be present in the DOM. The old DOM-based check caused the button to disappear on the next filter-change event after the user clicked it to turn highlighting off (because the onclick removes the highlight spans, leaving the DOM query returning zero). The button is now shown/hidden by the same condition as "Clear ALL filters".'},
...
(function() {
    ...
    prefilterToggleBtn.onclick = () => {
        prefilterHighlightEnabled = !prefilterHighlightEnabled;
        updatePrefilterButtonAppearance();
        if (typeof runFilter === 'function') runFilter();
        Lib.debug('highlight', `Prefilter highlights ${prefilterHighlightEnabled ? 'restored' : 'removed'}`);
    };
    ...
    unhighlightAllBtn.onclick = () => {
        filterHighlightEnabled = !filterHighlightEnabled;
        updateFilterHighlightButtonAppearance();
        if (typeof runFilter === 'function') runFilter();
        Lib.debug('highlight', `Filter highlights ${filterHighlightEnabled ? 'restored' : 'removed'}`);
    };
    ...
    function updateFilterButtonsVisibility() {
        ...
        // Show/hide Toggle highlighting button: visible whenever any filter is active,
        // regardless of whether highlight spans are currently present in the DOM.
        // (Basing visibility on DOM highlights was wrong: clicking the button removes the
        // highlight spans, so a subsequent filter-change event would hide the button even
        // though the user still needs it to toggle highlighting back on.)
        unhighlightAllBtn.style.display = (globalFilterActive || columnFiltersActive) ? 'inline-block' : 'none';
    ...
    function testRowMatch(row, ctx) {
        const { globalQuery, globalQueryRaw, globalRegex,
                ...
                filterHighlightEnabled: hlEnabled,
                prefilterHighlightEnabled: preHlEnabled } = ctx;

        // Reset previous filter highlights (critical for correct re-filtering)
        ...
        // Strip prefilter highlights from this clone when prefilter highlighting is disabled.
        // allRows/groupedRows carry prefilter spans baked-in at load time; cloning inherits them.
        if (!preHlEnabled) {
            row.querySelectorAll('.mb-pre-filter-highlight')
                .forEach(n => n.replaceWith(document.createTextNode(n.textContent)));
        }
    ...
    function runFilter() {
        ...
        const matchCtx = { globalQuery, globalQueryRaw, globalRegex,
                           isCaseSensitive, isRegExp, isExclude, colFilters: [],
                           filterHighlightEnabled, prefilterHighlightEnabled };
