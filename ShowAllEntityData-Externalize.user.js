...
// CHANGELOG — loaded at runtime from GitHub (see fetchRemoteData)
let changelog = [];

// APPLICATION HELP TEXT — loaded at runtime from GitHub (see fetchRemoteData)
const APP_HELP_TEXT = null;

(function() {
    'use strict';

    // ── REMOTE CONTENT (help text + changelog) ─────────────────────────────────
    // Both APP_HELP_TEXT and the changelog array are stored as companion files on
    // GitHub and fetched once per session (cached in GM storage for 1 hour).
    ...
    /**
     * Fetch a URL via GM_xmlhttpRequest, returns a Promise<string>.
     */
     ...
     * Returns { data: string, fromCache: boolean, error: string|null }
     */
    async function fetchCachedText(url, cacheKey) {
        try {
            const cached = GM_getValue(cacheKey, null);
            if (cached && (Date.now() - cached.ts) < REMOTE_CACHE_TTL_MS) {
                Lib.debug('remote', `Using cached content for ${cacheKey} (age ${Math.round((Date.now()-cached.ts)/1000)}s)`);
        ...
        try {
            ...
            Lib.debug('remote', `Fetched and cached remote content for ${cacheKey}`);
            return { data, fromCache: false, error: null };
        } catch (err) {
            Lib.warn('remote', `Fetch failed for ${url}: ${err.message}`);
            // Try stale cache as last resort
            try {
                const stale = GM_getValue(cacheKey, null);
                if (stale) {
                    Lib.warn('remote', `Using stale cache for ${cacheKey}`);
    ...
    /**
     * Fetch the remote changelog JSON and mutate the module-level changelog array in place.
     * Called once at startup; VZ_MBLibrary sees the updated array when the GM menu is triggered.
     */
    async function initRemoteChangelog() {
        const { data, error } = await fetchCachedText(REMOTE_CHANGELOG_URL, CACHE_KEY_CHANGELOG);
        if (!data) {
            Lib.warn('remote', `Changelog not available remotely: ${error}`);
            return;
        }
        try {
            const entries = JSON.parse(data);
            // Mutate in place so VZ_MBLibrary's closure sees the remote data
            changelog.length = 0;
            entries.forEach(e => changelog.push(e));
            Lib.debug('remote', `Remote changelog loaded: ${changelog.length} entries`);
        } catch (err) {
            Lib.warn('remote', `Failed to parse remote changelog JSON: ${err.message}`);
        }
    }

    // Kick off both fetches immediately (non-blocking)
    initRemoteChangelog();
    ...
    async function showAppHelp() {
        ...
        // ── Build the dialog shell (title bar + content area + footer) ───────────
        ...
        async function loadAndRender() {
            ...
            const { data, fromCache, error } = await fetchCachedText(REMOTE_HELP_URL, CACHE_KEY_HELP);
