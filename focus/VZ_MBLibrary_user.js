...
//     {version: '3.15.0+2026-02-24', description: 'Correct focus strategy for GM-menu-triggered dialogs (Settings + Changelog). Previous attempts used window.focus() which is blocked by browser security policy when called from extension-context code, and fixed setTimeout delays which are unreliable. Correct approach: (a) if document.hasFocus() is already true (dialog opened from a page button) call element.focus() immediately via setTimeout 0ms; (b) otherwise register a one-shot window "focus" event listener which fires exactly when the browser returns focus to the page after the menu closes — the callback removes itself then calls element.focus(). A 5-second safety timeout removes the listener if focus never returns (e.g. user switches tabs). Both Settings and Changelog dialogs use this identical pattern.'},
//     {version: '3.14.0+2026-02-24', description: 'Focus robustness for GM-menu-triggered dialogs: added window.focus() call before the setTimeout delay in both the Changelog searchInput focus and the Settings searchEl focus. window.focus() explicitly requests that the browser return focus to the page content area; without it, element.focus() is silently ignored because document.hasFocus() may still be false when the setTimeout callback fires after a GM menu click. Delay bumped from 200ms to 250ms for extra margin on slower systems.'},
...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        const _LIB_PREFS_DEFAULTS = {
            lib_content_font_size:   '1.05em',  // Content font-size in Changelog and App Help dialogs
            ...
            showModal: function() {
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    if (isCheck) {
                        ...
                        inputHtml = `<input type="number" id="${inputId}" value="${escAttr(this.values[key])}" min="${cfg.min || 0}" max="${cfg.max || 100}" style="width:90px;padding:4px 6px;border:1px solid #c8cdd5;border-radius:5px;font-size:0.9em;background:#fff;">`;
                    } else if (isPopupDialog) {
                        const fields       = cfg.fields || [];
                        const currentParts = String(this.values[key]).split('|');
                        const subInputsHtml = fields.map((fieldName, fi) => `<label style="display:flex;align-items:center;gap:6px;font-size:0.82em;"><span style="min-width:110px;text-align:right;color:#555;font-weight:600;white-space:nowrap;">${escAttr(fieldName)}:</span><input type="text" class="mb-pd-sub" data-master="${inputId}" data-index="${fi}" value="${escAttr(currentParts[fi] !== undefined ? currentParts[fi] : '')}" style="width:200px;padding:3px 6px;border:1px solid #bbb;border-radius:4px;font-size:0.95em;font-family:monospace;background:#fff;"></label>`).join('');
                    ...
                    col1.style.cssText = 'flex:0 0 450px;width:450px;min-width:0;padding:10px 14px;display:flex;align-items:center;flex-wrap:wrap;gap:6px;border-right:1px solid #e4e8ef;box-sizing:border-box;';
                    col1.innerHTML = `<label for="${inputId}" style="font-size:0.85em;font-weight:500;color:#334;cursor:pointer;white-space:nowrap;margin-right:4px;">${cfg.label}:</label>${inputHtml}`;

                    // Column 2 — default value
                    const col2 = document.createElement('div');
                    col2.style.cssText = 'flex:0 0 130px;width:130px;min-width:0;padding:10px;font-size:0.76em;color:#888;text-align:center;border-right:1px solid #e4e8ef;word-break:break-all;display:flex;align-items:center;justify-content:center;box-sizing:border-box;';
                ...
                container.innerHTML = `
                    ...
                    ">
                        <div style="flex:0 0 450px;width:450px;padding:6px 14px;border-right:1px solid #d0d5dd;box-sizing:border-box;">Setting</div>
                        <div style="flex:0 0 130px;width:130px;padding:6px 10px;text-align:center;border-right:1px solid #d0d5dd;box-sizing:border-box;">Default</div>
                        <div style="flex:1 1 0;padding:6px 14px;box-sizing:border-box;">Description</div>
                    </div>
                ...
                //
                // When the dialog is opened from a Tampermonkey GM menu click the
                // browser retains focus in its own chrome.  window.focus() is blocked
                // by browser security policy in that context, and a setTimeout of any
                // fixed length is unreliable.
                //
                // The correct approach:
                //   a) If the document already has focus (e.g. opened from a page
                //      button click) — focus immediately.
                //   b) Otherwise — register a one-shot 'focus' listener on window,
                //      which fires exactly when the browser returns focus to the page
                //      after the menu closes (triggered by the user moving the mouse
                //      over the page or clicking). A 5 s safety timeout cleans up the
                //      listener if focus never returns.
                {
                    const _focusSettingsSearch = () => {
                        const searchEl = document.getElementById(`${scriptId}-search`);
                        if (searchEl) searchEl.focus();
                    };
                    if (document.hasFocus()) {
                        setTimeout(_focusSettingsSearch, 0);
                    } else {
                        const _onWinFocus = () => {
                        ...
                        const _focusTimeout = setTimeout(() => {
                            window.removeEventListener('focus', _onWinFocus);
                        }, 5000);
        ...
        const changelogInterface = {
            ...
            show: function() {
                ...
                /**
                 * Wraps every case-insensitive occurrence of needle in the given text
                 * with a <mark> highlight span.  Safe to call with an empty needle.
                 * @param {string} text   Plain text (not yet HTML-encoded)
                 * @param {string} needle The search string (already lower-cased)
                ...
                //      listener if focus never returns.
                {
                    const _focusSearch = () => { if (searchInput) searchInput.focus(); };
                    if (document.hasFocus()) {
                        setTimeout(_focusSearch, 0);
                    } else {
                        const _onWinFocus = () => {
                            window.removeEventListener('focus', _onWinFocus);
                            clearTimeout(_focusClTimeout);
                            setTimeout(_focusSearch, 0);
                        };
                        const _focusClTimeout = setTimeout(() => {
                            window.removeEventListener('focus', _onWinFocus);
                        }, 5000);
                        window.addEventListener('focus', _onWinFocus);
                    }
                ...
                // Named reference so it can be removed in closeDialog, preventing
                // accumulation when the dialog is opened and closed multiple times.
