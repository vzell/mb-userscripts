...
// @downloadURL  https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypeWorks.user.js
// @updateURL    https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypeWorks.user.js
...
(function() {
    ...
    const LOG_PREFIX = '[VZ:discography-autoselect-works]';
    ...
    const DEFAULT_MAPPINGS_WORKS = [
        { regex: "http://brucebase.wikidot.com/song:.*", typeId: "921", description: "BruceBase song entry" },
        { regex: "https://www.springsteenlyrics.com/lyrics.php\?song=.*", typeId: "921", description: "BruceBase song entry" },
    ];
    ...
    function loadMappings() {
        try {
            const stored = localStorage.getItem(STORAGE_KEY_WORKS);
            if (stored) {
                return JSON.parse(stored) || DEFAULT_MAPPINGS_WORKS;
            }
        } catch (e) {
            log("Failed to load mappings from localStorage. Using defaults.", e);
        }
        return DEFAULT_MAPPINGS_WORKS;
    }

    function saveMappings(mappings) {
        try {
            localStorage.setItem(STORAGE_KEY_WORKS, JSON.stringify(mappings));
                ...
                mutation.addedNodes.forEach(node => {
                    ...
                    if (selectElement && lastCleanedUrlData) {
                        ...
                        setTimeout(() => {
                            ...
                            if (linkValueToSet) {
                                const selectedOption = LINK_TYPES_OPTIONS_WORKS.find(o => o.id === linkValueToSet);
                                if (selectedOption) {
