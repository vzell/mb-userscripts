...
//     {version: '2.6.0+2026-02-22', description: 'Keyboard event isolation for settings modal: a capture-phase keydown listener is registered on window as soon as the settings dialog opens and removed when it closes. The blocker calls stopPropagation() + preventDefault() for every key, preventing the host page and browser from acting on keyboard shortcuts while the dialog has focus. The only exception is when a keyboard_shortcut widget is actively capturing (data-capturing="true"): in that case the existing captureHandler on document (also capture phase) owns the event and the blocker steps aside. The previous bare Escape listener is replaced by Escape handling inside the blocker so no double-listener exists.'},
            ...
            showModal: function () {
                ...
                document.querySelectorAll(".section-header").forEach(header => {
                    header.addEventListener("click", () => {
                        ...
                        // Also handle popup_dialog sub-grid rows: on collapse always hide;
                        // on expand only restore if the toggle had them open.
                ...
                const closeDialog = () => {
                    // Remove the keyboard event blocker before the overlay is gone.
                    window.removeEventListener('keydown', dialogKeyBlocker, true);
                ...
                // While the settings dialog is open we must prevent page-level
                // (and, where the browser permits, browser-level) keyboard handlers
                // from reacting to any key pressed inside the dialog.
                //
                // Strategy: register a capture-phase keydown listener on window.
                // Capture phase runs *before* bubble phase, so this handler sees
                // the event before any document/window bubble-phase listener the
                // host page installed.  We call:
                //   - stopPropagation() — stops the event from travelling further
                //     through the capture *and* bubble chains after our handler.
                //   - preventDefault()  — suppresses the browser's built-in action
                //     (e.g. Ctrl+N opening a new window, Ctrl+F opening find bar).
                //
                // Exception — keyboard_shortcut capture mode:
                //   When a capture widget is active (data-capturing="true") the
                //   dedicated captureHandler already registered on *document* in
                //   capture phase owns the event (stopPropagation + preventDefault
                //   are called there).  We detect this situation and step aside so
                //   as not to swallow the event before captureHandler sees it.
                //   (window capture fires before document capture, so stepping aside
                //   means captureHandler still runs.)
                //
                // The listener is removed inside closeDialog for clean teardown.
                //
                ...
                function dialogKeyBlocker(e) {
                    // If a keyboard_shortcut widget is actively capturing, its own
                    // captureHandler (document, capture phase) handles everything —
                    // let the event through so that handler can see it.
                    if (overlay.querySelector('[data-capturing="true"]')) return;

                    // Block the event from reaching any page-level handler.
                    e.stopPropagation();
                    e.preventDefault();
                ...
                window.addEventListener('keydown', dialogKeyBlocker, true);
            }
        };
