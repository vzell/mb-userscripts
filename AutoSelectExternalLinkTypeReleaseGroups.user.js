...
(function() {
    ...
    const LOG_PREFIX = '[VZ:discography-autoselect-release-groups]';

    function log(...args) {
        if (DEBUG) console.log(LOG_PREFIX, ...args);
    }

    const STORAGE_KEY_RELEASE_GROUPS = 'MB_AutoSelect_Mappings_Release_Groups';

    const LINK_TYPES_OPTIONS_RELEASE_GROUPS = [
    ...
    const DEFAULT_MAPPINGS_RELEASE_GROUPS = [
    ...
    function loadMappings() {
        try {
            const stored = localStorage.getItem(STORAGE_KEY_RELEASE_GROUPS);
            if (stored) {
                return JSON.parse(stored) || DEFAULT_MAPPINGS_RELEASE_GROUPS;
            }
        } catch (e) {
            log("Failed to load mappings from localStorage. Using defaults.", e);
        }
        return DEFAULT_MAPPINGS_RELEASE_GROUPS;
    }

    function saveMappings(mappings) {
        try {
            localStorage.setItem(STORAGE_KEY_RELEASE_GROUPS, JSON.stringify(mappings));
                ...
                mutation.addedNodes.forEach(node => {
                    ...
                    if (selectElement && lastCleanedUrlData) {
                        ...
                        setTimeout(() => {
                            ...
                            if (linkValueToSet) {
                                const selectedOption = LINK_TYPES_OPTIONS_RELEASE_GROUPS.find(o => o.id === linkValueToSet);
                                if (selectedOption) {
    ...
    function renderMappings(modalBody, openEditModalCallback) {
        ...
        linkMappings.forEach((mapping, index) => {
            const option = LINK_TYPES_OPTIONS_RELEASE_GROUPS.find(opt => opt.id === mapping.typeId);
    ...
    function openEditModal(index, reEnableMainEscape) {
        ...
        const currentMapping = isEditing ? linkMappings[index] : { regex: '', typeId: LINK_TYPES_OPTIONS_RELEASE_GROUPS[0].id, description: '' };
        ...
        modalContainer.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select id="typeSelect" style="padding: 5px; border: 1px solid #ccc; width: 100%;">
                    ${LINK_TYPES_OPTIONS_RELEASE_GROUPS.map(opt =>
