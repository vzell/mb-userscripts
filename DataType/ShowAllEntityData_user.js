...
// CHANGELOG
let changelog = [
    {version: '9.66.0+2026-02-21', description: 'configSchema: all 6 sa_popup_* entries changed from type:"text" to type:"popup_dialog" and each gains a fields:[...] array naming every pipe-separated parameter (e.g. ["bg","border","borderRadius","padding","boxShadow","zIndex","fontFamily","minWidth","maxWidth"] for sa_popup_dialog_style). The library\'s new popup_dialog renderer replaces the plain text box with a hidden master <input> (pipe-joined, read by the save handler unchanged) plus an "✏️ Edit fields" toggle that reveals a flex sub-grid of individually labelled monospace text inputs, one per pipe-field. sa_popup_btn_gap gets fields:["gap"].'}, description: 'Progress bar label text shortened: "Loading page N of M... K rows — estimated X.Xs remaining" → "Loading page N of M... (K rows) - est. X.Xs" — rows count wrapped in parens, em-dash replaced with hyphen, "estimated … remaining" trimmed to "est. …" to save horizontal space.'}, description: 'Bug fix: progress-bar label text clipped at both ends. Root cause: fetchProgressLabel was a child of mb-fetch-progress-outer whose overflow:hidden clips anything wider than the track\'s computed width (absolute children do not contribute to width:auto, so the track stayed at min-width:420px). Fix: label moved out of fetchProgressOuter and appended directly to fetchProgressWrap instead. fetchProgressWrap gains position:relative and height:20px so it becomes the containing block for the absolute label. The label uses min-width:100% (covers at least the track width) with white-space:nowrap so long texts extend freely beyond the track on both sides without being clipped. overflow:hidden on fetchProgressOuter is preserved to correctly clip the fill bar at the track edges.'}, description: 'UI consolidation: per-page fetch progress text moved from mb-global-status-display into the mb-fetch-progress-label span that is overlaid inside the mb-fetch-progress-outer bar. fetchProgressLabel now uses absolute positioning with display:flex, mix-blend-mode:difference and filter:invert(1) so the text stays readable over both the coloured fill and the grey track. The redundant globalStatusDisplay.textContent and .style.color updates that mirrored the same "Loading page N of M…" message have been removed; globalStatusDisplay is cleared to "" at fetch-start. Final/error status messages in globalStatusDisplay are unchanged.'},
...
(function() {
    ...
    // position:relative turns this into the containing block for fetchProgressLabel
    // which is positioned absolute inside here (not inside fetchProgressOuter) so
    // the label is never clipped by overflow:hidden on the track.
    fetchProgressWrap.style.cssText = 'display:none; align-items:center; vertical-align:middle; position:relative; height:20px;';

    // Custom progress bar: outer track (fill only) + label overlaid as a sibling so it
    // is never clipped by overflow:hidden on the track.
    //
    // DOM structure inside fetchProgressWrap (position:relative):
    //   fetchProgressOuter  — the coloured track bar (overflow:hidden clips the fill)
    //     fetchProgressFill — animated coloured fill
    //   fetchProgressLabel  — text overlay, positioned absolute over the wrap,
    //                         free to be wider than the track without being clipped
    ...
    fetchProgressOuter.style.cssText = [
        ...
        'background:#e0e0e0',
        'border-radius:4px',
        'overflow:hidden',          // clips the fill bar at the track edges
    ...
    // Label is a sibling of fetchProgressOuter (NOT a child) so overflow:hidden on
    // the track does not clip it. The wrap itself is position:relative; the label
    // is absolute within the wrap, covering the full track height and centred.
    // min-width matches the track so short texts still fill the bar visually;
    // white-space:nowrap lets long texts extend beyond the track without wrapping
    // or being cut.
    ...
    fetchProgressLabel.style.cssText = [
        ...
        'top:0',
        'left:0',
        'min-width:100%',           // at least as wide as the track
        'height:100%',
        'display:flex',
        'align-items:center',
        'justify-content:center',
        ...
        'white-space:nowrap',
        'pointer-events:none',
        'mix-blend-mode:difference',    // keeps text readable over both fill and track
        'filter:invert(1)',             // paired with mix-blend-mode for max contrast
    ].join(';');

    fetchProgressOuter.appendChild(fetchProgressFill);
    // label goes into the wrap, not into the outer, so it escapes overflow:hidden
    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                // Drive the inline progress bar in the h1 controls line — label carries the full status text
