...
// CHANGELOG
let changelog = [
    {version: '9.63.0+2026-02-21', description: 'Progress bar replaced + redundant status removed: (1) Replaced custom span-based bar (mb-fetch-progress-outer/fill/label + mb-fetch-progress-wrap) with the classic div-based implementation from progressbar.js: mb-fetch-progress-container (300px × 26px div, display:none), mb-fetch-progress-bar (absolute fill div, width 0%, transition width 0.3s + background-color 0.3s), mb-fetch-progress-text (absolute centered label, font-size 10px, z-index 1). Fill colours remain light pastels: #ffcccc (red) → #ffe0b2 (orange) → #ccffcc (green). progressText shows "Estimated remaining time: Xs". progressContainer appended directly to controlsContainer. (2) Removed the redundant estimated-time text from mb-global-status-display during fetching — the subheader now only shows the short "Loading page N of M… (K rows)" text (no "— est. Xs remaining" suffix); the estimated time is exclusively displayed inside the progress bar via progressText.'},
...
(function() {
    ...
    // Implementation matches progressbar.js: container div + fill div + text div
    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                // Short page-count status in the subheader (no redundant est. time — that lives in the bar)
