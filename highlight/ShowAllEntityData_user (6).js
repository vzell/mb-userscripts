...
(function() {
    ...
    // Create toggle highlighting button (initially shows "Toggle highlighting", later shows prefilter info)
    const unhighlightAllBtn = document.createElement('button');
    unhighlightAllBtn.id = 'mb-toggle-highlight-btn';
    ...
    unhighlightAllBtn.title = 'Toggle all highlighting on/off (prefilter, global filter, and column filters)';

    /**
     * Update the toggle highlighting button text and appearance
     * @param {number} count - Number of prefiltered rows (0 if no prefilter)
     * @param {string} query - The prefilter query string
     */
    function updateToggleHighlightButton(count = 0, query = '') {
        prefilterInfo = { count, query };

        if (count > 0 && query) {
            // Show prefilter info in button text
            unhighlightAllBtn.textContent = `${count} row${count === 1 ? '' : 's'} prefiltered: "${query}"`;
            unhighlightAllBtn.title = 'Toggle prefilter highlighting on/off';
        } else {
            // Default text when no prefilter
            unhighlightAllBtn.textContent = '🎨 Toggle highlighting';
            unhighlightAllBtn.title = 'Toggle all highlighting on/off (prefilter, global filter, and column filters)';
        }

        // Update button appearance based on highlighting state
        updateHighlightButtonAppearance();
    }

    /**
     * Update button background color based on highlighting state
     */
    function updateHighlightButtonAppearance() {
    ...
    unhighlightAllBtn.onclick = () => {
        if (highlightingEnabled) {
            // Currently highlighted - save state and remove highlights
            saveHighlightState();
            ...
            updateHighlightButtonAppearance();
            Lib.info('highlight', 'All highlights removed (saved state for restoration)');
        } else {
            // Currently not highlighted - restore saved state
            restoreHighlightState();
            highlightingEnabled = true;
            updateHighlightButtonAppearance();
            Lib.info('highlight', 'Highlights restored');
        }
    };
    ...
    let savedHighlights = { hasContent: false };
    ...
    /**
     * Save the current highlight state before removing highlights
     * Captures highlighting parameters to enable accurate restoration
     */
    function saveHighlightState() {
        ...
        savedHighlights = {
            ...
            // Could save global and column filter params here too if needed
            hasContent: !!prefilterInfo.query
        };

        Lib.debug('highlight', `Saved highlight state: ${savedHighlights.hasContent ? 'has prefilter' : 'no prefilter'}`);
    }

    /**
     * Restore previously saved highlights by re-applying highlighting
     */
    function restoreHighlightState() {
        if (!savedHighlights.hasContent) {
            Lib.info('highlight', 'No saved highlight state to restore');
            return;
        }

        // Restore prefilter highlighting
        if (savedHighlights.prefilter) {
            const { query, caseSensitive, isRegex } = savedHighlights.prefilter;
        ...
        // Could restore global and column filter highlighting here too if needed
    ...
    function loadTableDataFromDisk(file, filterQueryRaw = '', isCaseSensitive = false, isRegExp = false) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                if (filterQueryRaw) {
                    // Update the toggle highlighting button with prefilter info
                    updateToggleHighlightButton(loadedRowCount, filterQueryRaw);
                    ...
                    highlightingEnabled = true;
                    savedHighlights = { hasContent: false };
                } else {
                    // No prefilter, reset button to default
                    updateToggleHighlightButton(0, '');
                    preFilterMsg.style.display = 'none';
                }
