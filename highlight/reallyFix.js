...
// CHANGELOG
let changelog = [
    {version: '9.26.3+2026-02-16', description: 'Critical Fix: Completely rewrote filter highlighting toggle logic. Now properly unwraps BOTH highlight spans AND wrapper spans when toggling off, ensuring clean DOM structure. When toggling on, runFilter() creates fresh highlighting on clean cells. This fixes the bug where global filter highlighting would never restore after toggling. Simplified restoreFilterHighlightState() to just call runFilter() since cleanup is now done in toggle off handler.'},
...
(function() {
    ...
    unhighlightAllBtn.onclick = () => {
        if (filterHighlightEnabled) {
            // Currently highlighted - save state and remove filter highlights
            saveFilterHighlightState();

            // Remove highlights and clean up wrapper spans created by highlightText
            document.querySelectorAll('table.tbl tbody tr').forEach(row => {
                row.querySelectorAll('td').forEach(td => {
                    // Check if this cell has any highlight or wrapper spans
                    const hasHighlightSpans = td.querySelector('.mb-global-filter-highlight, .mb-column-filter-highlight');
                    const hasWrapperSpans = td.querySelector('span');

                    if (hasHighlightSpans || hasWrapperSpans) {
                        // Save the text content
                        const textContent = td.textContent;

                        // Check if cell has important elements we need to preserve
                        const links = Array.from(td.querySelectorAll('a'));
                        const images = Array.from(td.querySelectorAll('img'));
                        const hasImportantElements = links.length > 0 || images.length > 0;

                        if (!hasImportantElements) {
                            // Simple cell - just replace with text content
                            td.innerHTML = '';
                            td.appendChild(document.createTextNode(textContent));
                        } else {
                            // Complex cell - need to preserve links/images
                            // Walk through and unwrap all span elements
    ...
    function restoreFilterHighlightState() {
        ...
        // The DOM has been cleaned up in the toggle off handler, so this will work correctly
