...
(function() {
    ...
    function restoreFilterHighlightState() {
        ...
        // Restore global filter highlighting
        if (savedFilterHighlights.globalFilter) {
            const { query } = savedFilterHighlights.globalFilter;
            Lib.debug('highlight', `Restoring global filter highlight for: "${query}"`);

            tables.forEach(table => {
                table.querySelectorAll('tbody tr').forEach(row => {
                    highlightText(row, query, false, -1, false, 'global');
                });
            });
        }

        // Restore column filter highlighting
        if (savedFilterHighlights.columnFilters.length > 0) {
            savedFilterHighlights.columnFilters.forEach(filter => {
                Lib.debug('highlight', `Restoring column filter highlight for column ${filter.index}: "${filter.query}"`);

                tables.forEach(table => {
                    table.querySelectorAll('tbody tr').forEach(row => {
                        highlightText(row, filter.query, filter.caseSensitive, filter.index, filter.isRegex, 'column');
                    });
                });
            });
        }
