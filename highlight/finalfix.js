...
// CHANGELOG
let changelog = [
    {version: '9.26.2+2026-02-16', description: 'Critical Fixes: (1) Fixed filter highlighting toggle - now properly cleans up DOM structure before restoring highlights, ensuring highlights reappear correctly when toggling on. Handles both simple text cells and complex cells with links/images. (2) Added tooltip to Stop button: "Stop the current data fetching process". (3) Fixed vertical alignment of status displays when loading directly from disk - added vertical-align:middle to prevent top-alignment issue.'},
...
(function() {
    ...
    stopBtn.title = 'Stop the current data fetching process';
    ...
    function restoreFilterHighlightState() {
        ...
        // First, ensure ALL highlights are completely removed (including wrapper spans)
        // This ensures a clean slate for re-highlighting
        document.querySelectorAll('table.tbl tbody tr').forEach(row => {
            row.querySelectorAll('td').forEach(td => {
                // Check if this cell has been modified by highlighting
                const hasHighlightSpans = td.querySelector('.mb-global-filter-highlight, .mb-column-filter-highlight');
                if (hasHighlightSpans || td.querySelector('span')) {
                    // Clean up by extracting all text content and replacing the cell's HTML
                    const textContent = td.textContent;
                    // Preserve links and other important elements by checking if cell has links
                    ...
                    if (links.length === 0 && images.length === 0) {
                        // Simple text cell - just clean up completely
                        td.innerHTML = '';
                        td.textContent = textContent;
                    } else {
                        // Complex cell with links/images - more careful cleanup
                        // Remove only highlight-related spans
                        td.querySelectorAll('span').forEach(span => {
                            if (span.classList.contains('mb-global-filter-highlight') ||
                                span.classList.contains('mb-column-filter-highlight') ||
                                span.classList.contains('mb-pre-filter-highlight')) {
                                span.replaceWith(document.createTextNode(span.textContent));
                            } else if (span.querySelector('.mb-global-filter-highlight, .mb-column-filter-highlight, .mb-pre-filter-highlight')) {
                                // This is a wrapper span created by highlightText
                                const tempDiv = document.createElement('div');
                                tempDiv.innerHTML = span.innerHTML;
                                // Remove highlight spans inside
                                tempDiv.querySelectorAll('.mb-global-filter-highlight, .mb-column-filter-highlight, .mb-pre-filter-highlight').forEach(h => {
                                    h.replaceWith(document.createTextNode(h.textContent));
                                });
                                span.replaceWith(...Array.from(tempDiv.childNodes));
                            }
                        });
                    }
                }
        ...
        Lib.debug('highlight', 'Cleaned up all filter highlights, now re-running filters to restore');

        // Now re-run the filter which will automatically re-apply highlighting
        if (typeof runFilter === 'function') {
            runFilter();
