...
(function() {
    ...
    const configSchema = {
        ...
        sa_global_filter_active_border_color: {
            label: "Global Filter Active Border Color",
            type: "color_picker",
            default: "#FFA500",
            description: "Border/shadow color of the global filter input when a filter string is active"
        },

        sa_subtable_filter_active_border_color: {
            label: "Sub-table Filter Active Border Color",
            type: "color_picker",
            default: "#008000",
            description: "Border/shadow color of sub-table filter inputs when a filter string is active"
        },
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function updateFilterButtonsVisibility() {
        ...
        // Show "Toggle highlighting" only when a filter string is present —
        // there is nothing to toggle without an active filter.
    ...
    async function showLoadFilterDialog(triggerButton = null) {
        ...
        const confirmLoad = async () => {
            ...
            // Validate regex BEFORE closing the dialog so the user can correct it in place.
            if (query && useRegex) {
                try {
                    new RegExp(query, useCase ? '' : 'i');
                    input.style.borderColor = '';  // reset any previous error highlight
                } catch (e) {
                    input.style.borderColor = 'red';
                    input.style.boxShadow = '0 0 0 2px rgba(255,0,0,0.35)';
                    await Lib.showCustomAlert(
                        `Invalid Regular Expression: ${e.message}\n\nPlease correct the pattern before loading.`,
                        '❌ Invalid Regex',
                        dialog
                    );
                    input.focus();
                    return; // Abort — keep dialog open for correction
                }
            } else {
    ...
    function runFilter() {
        ...
        const activeBorderColor = Lib.settings.sa_global_filter_active_border_color || '#FFA500';

        let globalRegex = null;
        if (globalQueryRaw && isRegExp) {
            try {
                globalRegex = new RegExp(globalQueryRaw, isCaseSensitive ? '' : 'i');
                filterInput.style.border = `2px solid ${activeBorderColor}`; // Valid regex — active colour
            } catch (e) {
                filterInput.style.border = '2px solid red';
                filterInput.style.boxShadow = '0 0 0 3px rgba(255,0,0,0.35)';
                if (filterStatusDisplay) {
                    filterStatusDisplay.textContent = `⚠️ Invalid regex: ${e.message}`;
                    filterStatusDisplay.style.color = 'red';
                }
                return; // Abort — leave current rows visible, border stays red as reminder
            }
        } else {
            filterInput.style.border = globalQueryRaw ? `2px solid ${activeBorderColor}` : '2px solid #ccc';
        }

        // Apply coloured glow to global filter when active
        filterInput.style.boxShadow = globalQueryRaw ? `0 0 2px 2px ${activeBorderColor}` : '';
    ...
    function createSubTableFilterContainer(categoryName, table) {
        ...
        // ── Highlight toggle button — hidden until a filter string is present ──
        ...
        // ── Clear ALL sub-table filters button — hidden until a filter string is present ──
        const clearAllSubTableBtn = document.createElement('button');
        clearAllSubTableBtn.id = `${pfx}-clear-all-btn`;
        clearAllSubTableBtn.type = 'button';
        clearAllSubTableBtn.title = `Clear sub-table filter and ALL column filters for "${categoryName}"`;
        clearAllSubTableBtn.textContent = '✕ Clear ALL filters';
        clearAllSubTableBtn.style.cssText = 'font-size:0.8em; padding:2px 6px; border-radius:4px; background:rgb(240,240,240); border:1px solid rgb(204,204,204); cursor:pointer; vertical-align:middle; display:none;';
        ...
        container.appendChild(clearAllSubTableBtn);
        ...
        function updateInputStyle() {
            const c = Lib.settings.sa_subtable_filter_active_border_color || '#008000';
            const hasValue = !!filterInput.value;
            filterInput.style.boxShadow  = hasValue ? `0 0 2px 2px ${c}` : '';
            filterInput.style.borderColor = hasValue ? c : 'rgb(204,204,204)';
            // Show action buttons only when a filter string is present
            highlightBtn.style.display        = hasValue ? 'inline-block' : 'none';
            clearAllSubTableBtn.style.display  = hasValue ? 'inline-block' : 'none';
        }
        ...
        function applySubFilter() {
            ...
            let regex = null;
            const activeColor = Lib.settings.sa_subtable_filter_active_border_color || '#008000';
            if (useRx) {
                try {
                    regex = new RegExp(`(${raw})`, useCase ? 'g' : 'gi');
                    filterInput.style.borderColor = activeColor;
                } catch (e) {
                    filterInput.style.borderColor = 'red';
                    filterInput.style.boxShadow = '0 0 0 3px rgba(255,0,0,0.35)';
                    // Show inline error in the h3 filter-status span (no alert)
                    const h3 = table?.previousElementSibling;
                    const statusEl = h3?.querySelector('.mb-filter-status');
                    if (statusEl) {
                        statusEl.textContent = `⚠️ Invalid regex: ${e.message}`;
                        statusEl.style.color = 'red';
                    }
                    return; // Abort — rows remain in current state, border stays red
                }
            } else {
            ...
            Array.from(table.querySelectorAll('tbody tr')).forEach(row => {
                ...
                let matchFound = false;
                if (useRx) {
                    // Reuse the already-compiled regex from Step 3 (reset lastIndex for 'g' flag)
                    matchFound = Array.from(row.cells).some(cell => {
                        regex.lastIndex = 0;
        ...
        // ── Clear ALL sub-table filters button handler ────────────────────────
        clearAllSubTableBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            // Clear the subtable filter text input and restore hidden rows / highlights
            filterInput.value = '';
            clearSubFilter();
            // Also clear all column filter inputs for this specific table
            if (table) {
                table.querySelectorAll('.mb-col-filter-input').forEach(inp => {
                    inp.value = '';
                    inp.style.boxShadow = '';
                });
                // Re-run the global filter so column-filter state is fully updated
