        ...
        const settingsInterface = {
            ...
            showModal: function() {
                ...
                const tableRows = Object.entries(configSchema).map(([key, cfg]) => {
                    ...
                    let inputHtml = `<input type="${isCheck ? 'checkbox' : 'text'}" id="${inputId}" ${valAttr} style="${isCheck ? '' : 'width: 120px;'} margin-left: 5px;">`;

                    if (isColor) {
                        inputHtml += `<button id="${inputId}-picker-btn" type="button" style="margin-left: 5px; cursor: pointer;" title="Open Color Picker">🎨</button>`;
                    }
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    if (cfg.type === "color_picker") {
                        const btn = document.getElementById(`${scriptId}-input-${key}-picker-btn`);
                        ...
                        btn.onclick = (e) => {
                            ...
                            if (typeof iro !== 'undefined') {
                                activePicker = new iro.ColorPicker(`#${scriptId}-picker-container`, {
                                    width: 150,
                                    color: input.value || cfg.default
                                });

                                activePicker.on('color:change', function(color) {
