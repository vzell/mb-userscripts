    ...
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true) {
        ...
        const settingsInterface = {
            ...
            showModal: function () {
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    if (cfg.type === 'divider') {
                        ...
                        tableRows += `
                            <tr class="section-header" data-section="${key}"
                                style="cursor:pointer;background:linear-gradient(to right,#888,#bbb,#888);color:white;">
                    ...
                    if (isColor) {
                        inputHtml += `
                            <button id="${inputId}-picker-btn"
                                    type="button"
                                    style="margin-left:5px;width:26px;height:26px;cursor:pointer;">
                                🎨
                            </button>`;
                    }
                    ...
                    tableRows += `
                        <tr class="config-row" data-section="${currentSection || ''}">
                            <th style="text-align:left;width:240px;vertical-align:top;">
                                ${cfg.label}: ${inputHtml}
                            </th>
                            <td style="width:130px;text-align:center;opacity:.6;">
                                ${cfg.default}
                            </td>
                            <td style="white-space:normal;word-break:break-word;min-width:350px;">
                                ${cfg.description}
                            </td>
                        </tr>`;
                });
                ...
                container.innerHTML = `
                    <div id="${scriptId}-drag-header" style="cursor:move;user-select:none;">
                        <p style="text-align:right;margin:0;">
                            <a id="${scriptId}-reset" style="cursor:pointer;font-weight:bold;">RESET</a> |
                            <a id="${scriptId}-close" style="cursor:pointer;font-weight:bold;">CLOSE</a>
                        </p>
                    ...
                    <div style="flex:1;overflow-y:auto;background:#f2f2f2;">
                    ...
                    <div id="${scriptId}-picker-container"
                         style="position:absolute;display:none;background:#eee;
                                border:1px solid #999;padding:10px;z-index:10001;
                ...
                document.addEventListener("mousemove", e => {
                    ...
                    if (isResizing) {
                        ...
                        container.style.width =
                            Math.max(700, Math.min(window.innerWidth * 0.95, newWidth)) + 'px';

                        container.style.height =
                            Math.max(450, Math.min(window.innerHeight * 0.9, newHeight)) + 'px';
                    }
                });
