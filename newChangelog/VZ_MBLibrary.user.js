...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        // loggerInterface via the closure; since they are async and only ever called
        // after the constructor returns, loggerInterface will already be initialised.
        ...
        const settingsInterface = {
            ...
            showModal: function () {
                ...
                // Reads all current form-input values into a plain object.
                const takeSnapshot = () => {
                    const snap = {};
                    for (const key in configSchema) {
                ...
                // Captured once when the modal first opens — the baseline for dirty-checking.
                ...
                // Guarded close — asks the user to confirm only when there are unsaved changes.
        ...
        const changelogInterface = {
            show: function () {
                ...
                /**
                 * Render a single item (string or {text, sub[]}) as an <li>.
                 * Sub-items become a nested <ul>.
                 * @param {string|{text:string,sub:string[]}} item
                 * @returns {string} <li> HTML
                 */
                const renderItem = item => {
                    if (typeof item === 'string') {
                        return `<li style="margin:2px 0;color:#333;">${item}</li>`;
                    }
                    const subHtml = (item.sub || [])
                        .map(s => `<li style="margin:1px 0;color:#555;">${s}</li>`)
                        .join('');
                    return `
                        <li style="margin:2px 0;color:#333;">${item.text}
                            ${subHtml ? `<ul style="margin:3px 0 3px 16px;padding:0;list-style:circle;">${subHtml}</ul>` : ''}
                        </li>`;
                };

                /**
                 * Build the document HTML from the current _changelog array.
                 * Supports both the new {version, date, sections[]} format and the
                 * legacy {version, description} flat-string format for backward compat.
                ...
                const buildContent = () => {
                    ...
                    _changelog.forEach(entry => {
                        // ── version / date header ──────────────────────────────
                        const versionLabel = entry.date
                            ? `${entry.version} <span style="font-weight:400;color:#888;">– ${entry.date}</span>`
                            : entry.version;
                        ...
                        if (Array.isArray(entry.sections)) {
                            entry.sections.forEach(section => {
                                html += `
                                    <div style="padding:6px 16px 4px 16px;background:#fff;">
                                        <p style="
                                            margin:0 0 3px 0;
                                            font-size:0.82em;
                                            font-weight:700;
                        ...
                        // ── legacy flat description (backward compat) ──────────
                            ...
                            const text = entry.description
                                || (Array.isArray(entry.description_list)
                                    ...
                                    : '');
                            html += `
                                <div style="padding:6px 16px 8px 16px;background:#fff;">
                                    <p style="margin:0;font-size:0.88em;color:#444;line-height:1.5;">${text}</p>
                                </div>`;
                        }

                        // ── bottom divider ─────────────────────────────────────
                        html += `<hr style="margin:0;border:none;border-top:1px solid #e0e4ea;">`;
                    });
                    return html;
                };
                ...
                refreshLink.addEventListener('click', async () => {
                    ...
                    if (data) {
                        try {
                            ...
                            entriesEl.innerHTML = buildContent();
                            updateMetaLine();
        ...
        const setupMenus = function() {
            if (typeof GM_registerMenuCommand !== 'undefined') {
                ...
                // Only register the ChangeLog menu item here when using a *static*
                // changelog (no remoteConfig provided).  When remoteConfig is set the
                // item is registered by initRemoteContent() after the async fetch so
                // that it is only visible once real data is available.
         ...
         * ChangeLog GM menu command.  Called once at construction time; the result
         * is fully asynchronous and does not block script startup.
         */
        const initRemoteContent = async function() {
            if (!remoteConfig || !remoteConfig.changelogUrl) {
                // No remote config — static changelog (if any) is the final source.
