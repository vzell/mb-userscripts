...
//     {version: '3.2.0+2026-02-23', description: 'Remote content support moved into library: fetchRemoteText and fetchCachedText (GM_xmlhttpRequest-backed, TTL cache via GM_setValue/GM_getValue) are now exposed on the public API. Constructor accepts new optional remoteConfig parameter ({ changelogUrl, cacheKeyChangelog, cacheTtlMs }) to load the changelog JSON from GitHub at startup; after a successful fetch the library registers the ChangeLog GM menu item itself. This eliminates the need for every consumer script to duplicate the remote-fetch boilerplate and fixes the race condition where setupMenus ran before the async fetch completed.'},
...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        /** Confirm-style dialog (Cancel + OK buttons). */
        ...
        const settingsInterface = {
            ...
            applyColorPreview: function(inputId, color) {
                ...
                if (input) { input.style.backgroundColor = color; input.style.color = this.getContrastYIQ(color); }
            ...
            showModal: function () {
                ...
                const savedSize = GM_getValue(sizeKey, { width: 920, height: 680 });
                ...
                Object.assign(container.style, {
                    ...
                    minWidth: '720px', minHeight: '480px', maxWidth: '95vw', maxHeight: '92vh',
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    if (cfg.type === 'divider') {
                        ...
                        tableRows += `
                            <tr class="section-header" data-section="${key}"
                                style="cursor:pointer;color:white;border-bottom:1px solid rgba(255,255,255,0.2);">
                                <td colspan="3" style="padding:10px 14px;font-weight:600;background:linear-gradient(to right,#6c7a89,#8fa3b5,#6c7a89);">
                                    ▼ ${cfg.label}
                                </td>
                            </tr>`;
                        return;
                    ...
                    if (isCheck) {
                        ...
                        const subInputsHtml = fields.map((fieldName, fi) => `
                            <label style="display:flex;align-items:center;gap:5px;font-size:0.82em;">
                                ...
                                <input type="text" class="mb-pd-sub" data-master="${inputId}" data-index="${fi}"
                                       value="${escAttr(currentParts[fi] !== undefined ? currentParts[fi] : '')}"
                                       style="width:200px;padding:2px 5px;border:1px solid #bbb;border-radius:3px;font-size:0.95em;font-family:monospace;">
                            </label>`).join('');

                        inputHtml = `
                            <input type="hidden" id="${inputId}" value="${escAttr(this.values[key])}">
                            <button type="button" id="${inputId}-toggle"
                                    style="font-size:0.8em;padding:2px 8px;cursor:pointer;border:1px solid #aaa;border-radius:4px;background:#f0f0f0;vertical-align:middle;">
                                ✏️ Edit fields
                            </button>`;
                        subgridRow = `
                            <tr id="${inputId}-subgrid" class="mb-pd-subgrid"
                                data-section="${currentSection || ''}" data-pd-open="false"
                                style="display:none;background:#eef2f8;border-bottom:1px solid #d8dfe8;">
                                <td colspan="3" style="padding:10px 20px 12px 20px;">
                                    <div style="display:flex;flex-wrap:wrap;gap:8px 24px;align-items:center;">${subInputsHtml}</div>
                                </td>
                            </tr>`;
                    } else if (isKeyboardShortcut) {
                        inputHtml = `
                            <input type="text" id="${inputId}" ${valAttr} readonly
                                   style="width:130px;padding:3px 6px;border:1px solid #ccc;border-radius:4px;font-family:monospace;cursor:default;">
                            <button type="button" id="${inputId}-capture-btn" data-capturing="false"
                                    style="margin-left:5px;font-size:0.8em;padding:2px 8px;cursor:pointer;border:1px solid #aaa;border-radius:4px;background:#f0f0f0;vertical-align:middle;">
                                🎹 Capture
                            </button>`;
                    } else {
                    ...
                    tableRows += `
                        <tr class="config-row" data-section="${currentSection || ''}"
                            style="background:${stripe};border-bottom:1px solid #e3e6ea;transition:background 0.15s ease;">
                            <th style="text-align:left;width:240px;vertical-align:top;padding:8px 10px;font-weight:500;">
                                ${cfg.label}: ${inputHtml}
                            </th>
                            <td style="width:130px;text-align:center;opacity:.5;padding:8px 10px;word-break:break-all;font-size:0.8em;">
                ...
                container.innerHTML = `
                    <div id="${scriptId}-drag-header" style="cursor:move;user-select:none;margin-bottom:8px;">
                        ...
                        <div style="display:flex;align-items:center;gap:8px;margin-top:6px;">
                            <input id="${scriptId}-search"
                                   ...
                                   style="flex:1;padding:7px 10px;border-radius:6px;border:1px solid #ccc;">
                            <button type="button" id="${scriptId}-collapse-all-btn"
                                    style="white-space:nowrap;padding:5px 10px;font-size:1.0em;cursor:pointer;border:1px solid #aaa;border-radius:4px;background:#f0f0f0;flex-shrink:0;">
                                ⬆ Collapse all sections
                            </button>
                        </div>
                    </div>

                    <div style="flex:1;overflow-y:auto;overflow-x:hidden;background:#f9f9fb;border-radius:8px;">
                    ...
                    <div style="position:sticky;bottom:0;background:#fff;padding:14px 0 6px 0;text-align:right;border-top:2px solid #cdd;">
                        <button id="${scriptId}-save-btn"
                            style="padding:9px 20px;font-weight:600;border-radius:8px;border:1px solid #4CAF50;background:#4CAF50;color:white;cursor:pointer;transition:all 0.15s ease;">
                            SAVE
                        </button>
                    </div>
                    ...
                    <div id="${scriptId}-picker-container"
                         style="position:absolute;display:none;background:#fff;border:1px solid #ccc;padding:10px;z-index:10001;box-shadow:0 8px 25px rgba(0,0,0,0.2);">
                    </div>
                `;
                ...
                document.addEventListener("mousemove", e => {
                    ...
                    if (isResizing) {
                        ...
                        container.style.width  = Math.max(720, Math.min(window.innerWidth  * 0.95, e.clientX - rect.left)) + 'px';
                        container.style.height = Math.max(480, Math.min(window.innerHeight * 0.92, e.clientY - rect.top))  + 'px';
                    }
                });

                document.addEventListener("mouseup", () => {
                    if (isResizing) { GM_setValue(sizeKey, { width: container.offsetWidth, height: container.offsetHeight }); }
                ...
                function applySectionCollapse(header, collapsed) {
                    ...
                    if (collapsed) { header.classList.add('collapsed'); } else { header.classList.remove('collapsed'); }
                    header.innerHTML = header.innerHTML.replace(collapsed ? '▼' : '▶', collapsed ? '▶' : '▼');
                    ...
                    document.querySelectorAll(`.mb-pd-subgrid[data-section="${section}"]`)
                        .forEach(row => {
                            ...
                            else if (row.dataset.pdOpen === 'true') { row.style.display = ''; }
                        });
                }
                ...
                document.querySelectorAll('.section-header').forEach(header => { applySectionCollapse(header, true); });

                let allSectionsCollapsed = true;
                document.getElementById(`${scriptId}-collapse-all-btn`).textContent = '⬇ Uncollapse all sections';
                document.getElementById(`${scriptId}-collapse-all-btn`).addEventListener('click', () => {
                    allSectionsCollapsed = !allSectionsCollapsed;
                    document.querySelectorAll('.section-header').forEach(header => { applySectionCollapse(header, allSectionsCollapsed); });
                    document.getElementById(`${scriptId}-collapse-all-btn`).textContent =
                        allSectionsCollapsed ? '⬇ Uncollapse all sections' : '⬆ Collapse all sections';
                });
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    /** @param {KeyboardEvent} e @returns {string|null} */
                    function buildShortcutString(e) {
                        ...
                        const displayKey = (rawKey.length === 1 && /[a-z]/i.test(rawKey)) ? rawKey.toUpperCase() : rawKey;
                    ...
                    captureBtn.addEventListener('click', () => {
                        if (captureBtn.dataset.capturing === 'true') { exitCaptureMode(null); } else { enterCaptureMode(); }
                    });
                });
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    input.addEventListener('input', e => {
                        if (/^#[0-9A-F]{6}$/i.test(e.target.value)) { this.applyColorPreview(inputId, e.target.value); }
                    });
                    btn.onclick = (e) => {
                        e.stopPropagation();
                        ...
                        if (typeof iro !== 'undefined') {
                            const activePicker = new iro.ColorPicker(`#${scriptId}-picker-container`, { width: 180, color: input.value || cfg.default });
                            activePicker.on('color:change', (color) => { input.value = color.hexString; this.applyColorPreview(inputId, color.hexString); });
                        } else { pickerContainer.textContent = "iro.js missing"; }
                    };
                });
                ...
                const tryCloseDialog = async () => {
                    if (hasUnsavedChanges()) {
                        const confirmed = await showCustomConfirm('You have unsaved changes.\nDiscard changes and close the settings dialog?', 'Unsaved Changes');
                        if (!confirmed) return;
                    }
                    closeDialog();
                };
                ...
                const closeDialog = () => {
                    ...
                    setTimeout(() => { if (document.body.contains(overlay)) document.body.removeChild(overlay); }, 150);
                };
                ...
                window.addEventListener("keydown", e => { if (e.key === "Escape") tryCloseDialog(); });
            }
        };
        ...
        const changelogInterface = {
            show: function () {
                 ...
                 * Recursively nests sub-items.
                 * @param {string|{text:string,sub:string[]}} item
                 ...
                 *
                 * Markdown-inspired output format:
                 *   ## 9.89.0 – 2026-02-23
                 *   ### 🚀 Improvements
                 *   - Help dialog: Force refresh
                 *     - Added 🔄 Force refresh link in title bar
                 *   ### 🧹 Cleanup
                 *   - Removed popup CSS configuration options
                 *
                ...
                const buildContent = (filter = '') => {
                    ...
                    /** True when the entry's serialised JSON contains the needle. */
                    ...
                    _changelog.forEach(entry => {
                        ...
                        // ── legacy flat description (backward compat) ──────────
                            ...
                            const text = entry.description
                                || (Array.isArray(entry.description_list) ? entry.description_list.join(' ') : '');
                            html += `
        ...
        return {
            ...
            showSettings: () => settingsInterface.showModal(),
