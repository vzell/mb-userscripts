...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function createSubTableFilterContainer(categoryName, table) {
        ...
        // ── "Clear ALL" button (inside STF panel) ────────────────────────────
        // Visible only while the STF input contains text.  Clicking it clears the
        // STF input, restores rows hidden by the STF filter, removes STF highlights,
        // clears all column-level filter inputs for this table, and re-runs the
        // global filter so every view is fully consistent.
        const clearAllStfBtn = document.createElement('button');
        clearAllStfBtn.id = `${pfx}-clear-all-btn`;
        clearAllStfBtn.type = 'button';
        clearAllStfBtn.title = 'Clear sub-table filter AND all column filters for this table';
        // Create red ✗ symbol
        ...
        clearAllStfBtn.appendChild(document.createTextNode('Clear ALL'));
        // Initially hidden — shown by updateClearAllStfBtnVisibility() when STF has text
        ...
        clearAllStfBtn.addEventListener('click', (e) => {
            e.stopPropagation();
            // 1. Clear the STF input and restore hidden rows / highlights
            filterInput.value = '';
            clearSubFilter();
            // 2. Clear all column filter inputs for this table
            if (table) {
            ...
            // 3. Re-run the global filter so column-filter row visibility is refreshed
            if (typeof runFilter === 'function') runFilter();
            Lib.debug('filter', `STF "Clear ALL" clicked for "${categoryName}": STF + column filters cleared`);
        });
