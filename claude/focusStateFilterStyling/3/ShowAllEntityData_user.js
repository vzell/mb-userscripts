        ...
        document.querySelectorAll('.mb-subtable-clear-btn').forEach(btn => {
            ...
            if (h3) {
                ...
                if (table && table.tagName === 'TABLE') {
                    ...
                    btn.style.display = tableHasColFilters ? 'inline-block' : 'none';

                    // Also drive the per-subtable highlight-toggle button visibility:
                    // visible whenever the subtable filter input OR any column filter is active.
    ...
    function addColumnFilterRow(table) {
        ...
        Array.from(originalHeader.cells).forEach((cell, idx) => {
            ...
            clear.onclick = (e) => {
                e.preventDefault();
                e.stopPropagation();
                // Behave like Escape first press: if the focus prefix is present
                // (the input currently has focus), retain the prefix and clear only
                // the user-entered portion so the cursor stays in the field.
                // If the prefix is absent (input not focused), clear entirely and
                // strip active-filter background so the field is fully reset.
                const _pfx = Lib.settings.sa_col_filter_focus_prefix ?? '🔍 ';
                if (input.value.startsWith(_pfx)) {
                    // Input is focused — keep prefix, wipe user text only
                    input.value = _pfx;
                    input.setSelectionRange(_pfx.length, _pfx.length);
                } else {
                    input.value = '';
                    removeColFilterFocusStyle(input);
                }
