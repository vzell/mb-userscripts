                    ...
                    doc.querySelectorAll('table.tbl').forEach(table => {
                        ...
                        table.querySelectorAll('tbody tr:not(.explanation)').forEach(row => {
                            if (row.cells.length > 1) {
                                ...
                                // 1b. Build a name-keyed map from primary extractor outputs and run
                                //     synthetic-column extractors (second pass).
                                //     Source cells are located by synthetic column name rather than
                                //     DOM index, so no colIdx bookkeeping is required here.
                                ...
                                // 5. Append synthetic cells from synthetic-column extractors (second pass)
                                syntheticExtractorCells.forEach(cells => cells.forEach(td => newRow.appendChild(td)));

                                // 6. Append MB-Name / Comment when extractMainColumn is active
                                if (mainColIdx !== -1) {
                        ...
                        tableBody.childNodes.forEach(node => {
                            if (node.nodeName === 'TR') {
                                if (node.classList.contains('subh')) {
                                    ...
                                    // Append synthetic cells from synthetic-column extractors (second pass)
                                    syntheticExtractorCells.forEach(cells => cells.forEach(td => newRow.appendChild(td)));
