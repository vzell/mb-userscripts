...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    style.textContent = `
        ...
        #mb-col-uniq-dropdown.mb-uniq-visible {
            display: block;
        }
        .mb-col-uniq-item {
            padding: 5px 10px;
     ...
     */
    // -----------------------------------------------------------------------
    // Unique-values dropdown ("📊") — shared singleton panel
    // -----------------------------------------------------------------------

    /** @type {HTMLDivElement|null} Lazily created singleton dropdown element. */
    let _uniqDropdownEl = null;

    /** @type {HTMLSpanElement|null} The 📊 button that opened the current dropdown. */
    let _uniqDropdownOwnerBtn = null;

    /**
     * Returns (creating on first call) the singleton #mb-col-uniq-dropdown element.
     * @returns {HTMLDivElement}
     */
    function getUniqDropdownEl() {
        if (_uniqDropdownEl) return _uniqDropdownEl;
        ...
        document.addEventListener('mousedown', (e) => {
            if (!_uniqDropdownEl) return;
            if (!_uniqDropdownEl.contains(e.target) &&
                !e.target.classList.contains('mb-col-uniq-btn')) {
                closeUniqDropdown();
            }
        }, true);

        // Close on scroll of the table (header moving away from the button)
        document.addEventListener('scroll', () => closeUniqDropdown(), { passive: true, capture: true });
    ...
    /**
     * Closes the unique-values dropdown and clears the active-button marker.
     */
    function closeUniqDropdown() {
        if (!_uniqDropdownEl) return;
        _uniqDropdownEl.classList.remove('mb-uniq-visible');
        if (_uniqDropdownOwnerBtn) {
            _uniqDropdownOwnerBtn.classList.remove('mb-col-uniq-active');
            _uniqDropdownOwnerBtn = null;
        }
    }

    /**
     * Opens (or toggles) the unique-values dropdown for a column.
     *
     * Collects the distinct, non-empty text values present in `colIndex` of
     * every visible tbody row, sorts them case-insensitively, and renders them
     * in a scrollable panel anchored below `btn`.  Clicking or pressing Enter
     * on an item writes the value into the corresponding per-column filter
     * input and calls runFilter().
     *
     * Keyboard support inside the open panel:
     *   ArrowDown / ArrowUp  — move focus one item
     *   Home / End           — jump to first / last item
     *   Enter                — select focused item
     *   Escape               — close without selecting
     *   Any printable char   — scroll the first item starting with that char into view
     *
     * @param {HTMLSpanElement}    btn       - The 📊 button that was activated.
     * @param {HTMLTableElement}   table     - The owning <table> element.
     * @param {number}             colIndex  - Zero-based column index.
     */
    function openColumnUniqueValuesDropdown(btn, table, colIndex) {
        const dropdown = getUniqDropdownEl();

        // Toggle: close if already open for this button
        if (_uniqDropdownOwnerBtn === btn && dropdown.classList.contains('mb-uniq-visible')) {
            closeUniqDropdown();
            return;
        }

        // Mark previous owner as inactive before switching
        if (_uniqDropdownOwnerBtn && _uniqDropdownOwnerBtn !== btn) {
            _uniqDropdownOwnerBtn.classList.remove('mb-col-uniq-active');
        }
        _uniqDropdownOwnerBtn = btn;
        ...
        const sortedValues = Array.from(seen).sort((a, b) =>
            a.toLowerCase().localeCompare(b.toLowerCase())
        );

        // ---- Build dropdown content ----------------------------------------
        dropdown.innerHTML = '';
        dropdown.removeEventListener('keydown', dropdown._uniqKeyHandler);

        if (sortedValues.length === 0) {
            const empty = document.createElement('div');
            empty.className = 'mb-col-uniq-empty';
            ...
            sortedValues.forEach((val) => {
                ...
                item.addEventListener('mousedown', (e) => {
                    e.preventDefault(); // prevent blur on filter input
                });
                item.addEventListener('click', () => {
                    applyUniqValue(val, table, colIndex);
                    closeUniqDropdown();
                });
        ...
        const items = () => Array.from(dropdown.querySelectorAll('.mb-col-uniq-item'));

        const setFocus = (idx) => {
            const all = items();
            all.forEach(el => el.classList.remove('mb-uniq-focused'));
            if (idx < 0 || idx >= all.length) { focusedIdx = -1; return; }
            focusedIdx = idx;
            const target = all[idx];
            target.classList.add('mb-uniq-focused');
        ...
        const keyHandler = (e) => {
            ...
            if (e.key === 'ArrowDown') {
                ...
                if (focusedIdx >= 0 && all[focusedIdx]) {
                    applyUniqValue(all[focusedIdx].textContent, table, colIndex);
                    closeUniqDropdown();
                }
                ...
                const targetIdx = all.findIndex(el =>
                    el.textContent.toLowerCase().startsWith(ch)
                );
                if (targetIdx !== -1) setFocus(targetIdx);
            }
        };
        dropdown._uniqKeyHandler = keyHandler;
        dropdown.addEventListener('keydown', keyHandler);

        // Also listen on the owning button (it retains focus while dropdown is open)
        btn._uniqKeyHandler = keyHandler;
        btn.addEventListener('keydown', btn._uniqKeyHandler);
        btn.addEventListener('blur', function onBtnBlur() {
            // Give the dropdown a tick to receive focus before closing
            setTimeout(() => {
        ...
        dropdown.classList.add('mb-uniq-visible');

        // Preferred: left-align with button, open downward
        let top  = rect.bottom + 2;
        let left = rect.left;

        // Flip upward if not enough space below
        const dropH = Math.min(320, sortedValues.length * 28 + 8);
        if (top + dropH > viewH - 8 && rect.top > dropH) {
            top = rect.top - dropH - 2;
        }

        // Nudge left if clipped on right
        const dropW = Math.min(420, dropdown.offsetWidth || 200);
        if (left + dropW > viewW - 8) {
            left = Math.max(8, viewW - dropW - 8);
        }

        dropdown.style.top  = `${top}px`;
        dropdown.style.left = `${left}px`;

        Lib.debug('filter', `Unique-values dropdown for col ${colIndex}: ${sortedValues.length} values`);
    }

    /**
     * Writes `value` into the per-column filter input for `colIndex` in `table`
     * and triggers the filter pass.  Respects the decorative focus-prefix
     * convention (getFilterFocusPrefix / applyFilterFocusStyle).
     *
     * @param {string}           value     - The value to apply as a filter.
     * @param {HTMLTableElement} table     - The owning <table> element.
     * @param {number}           colIndex  - Zero-based column index.
     */
    function applyUniqValue(value, table, colIndex) {
        // Find the matching column filter input for this table + column
        const filterRow = table.querySelector('thead tr.mb-col-filter-row');
        if (!filterRow) return;
        const input = filterRow.querySelector(`.mb-col-filter-input[data-col-idx="${colIndex}"]`);
        if (!input) return;

        const prefix = getFilterFocusPrefix();
        input.value = prefix + value;
        // Sync active-background styling
        applyFilterFocusStyle(input, 'Filter on this column…', Lib.settings.sa_col_filter_focus_bg || '#fffde7');
        removeFilterFocusStyle(input, '…', true);
        runFilter();
        Lib.debug('filter', `Unique-value filter applied on col ${colIndex}: "${value}"`);
    }
    ...
    function makeTableSortableUnified(table, sortKey) {
        ...
        headers.forEach((th, index) => {
            ...
            // Create the 📊 unique-values button (right-aligned via float: right)
            const uniqBtn = document.createElement('span');
            uniqBtn.className = 'mb-col-uniq-btn';
            uniqBtn.textContent = '📊';
            uniqBtn.title = 'Show unique values — click or Enter to apply as column filter';
            uniqBtn.setAttribute('tabindex', '0');
            uniqBtn.setAttribute('role', 'button');
            uniqBtn.setAttribute('aria-label', `Unique values for column ${colName}`);

            uniqBtn.addEventListener('click', (e) => {
                e.preventDefault();
                e.stopPropagation();
                openColumnUniqueValuesDropdown(uniqBtn, table, index);
            });
            uniqBtn.addEventListener('keydown', (e) => {
            ...
            // The 📊 button floats right; it must be inserted BEFORE the text
            // node so CSS float places it at the right edge of the th.
            th.style.position = 'relative'; // anchor for any absolute children
            th.appendChild(uniqBtn);
