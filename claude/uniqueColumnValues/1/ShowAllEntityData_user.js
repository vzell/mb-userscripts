...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    //
    // Every column header gets a right-floated "📊" button injected by
    // makeTableSortableUnified.  Clicking it opens a scrollable panel showing
    // every distinct visible cell value for that column.  Selecting an item
    // writes it straight into the column's filter input and calls runFilter().
    // =========================================================================
     ...
     * and wires up the global close-on-outside-click and close-on-scroll listeners.
     *
    ...
    function getUniqDropEl() {
        ...
        // Close when anything outside the panel (or a 📊 button) is clicked
        document.addEventListener('mousedown', (ev) => {
            if (!_uniqDropEl || !_uniqDropOwner) return;
            if (ev.target === _uniqDropOwner) return; // handled by the button's own click
            if (_uniqDropEl.contains(ev.target)) return;
            closeUniqDrop();
        }, true);

        // Close whenever the page scrolls (header drifts away from panel)
        document.addEventListener('scroll', () => closeUniqDrop(), { passive: true, capture: true });
     ...
     *
     * Gathers the distinct non-empty textContent values from every *visible*
     ...
     *
     * Keyboard navigation while the panel is open (focus stays on `btn`):
    ...
    function openUniqDrop(btn, table, colIndex) {
        ...
        // Toggle: if this button already owns the open panel, close it
        ...
        // ------------------------------------------------------------------
        // Collect unique, non-empty values from visible tbody rows
        // ------------------------------------------------------------------
        const seen = new Set();
        ...
        // ------------------------------------------------------------------
        drop.innerHTML = '';
        // Remove any previously attached keydown handler
        if (drop._uniqKH) {
            drop.removeEventListener('keydown', drop._uniqKH);
            delete drop._uniqKH;
        }
        ...
        if (vals.length === 0) {
            ...
            vals.forEach(v => {
                ...
                item.textContent = v;
                item.title = v;
                // mousedown: prevent the click from blurring the filter input
        ...
        // ------------------------------------------------------------------
        // Keyboard navigation — all events are caught on `btn` which keeps
        // focus so that the user can keep pressing keys without the panel
        ...
        const moveFocus = (idx) => {
            ...
            focIdx = idx;
            items[idx].classList.add('mb-uniq-focused');
        ...
        // Nudge leftward if clipped on right
        if (left + dropW > vw - 6) {
     ...
     * of `table` and triggers an immediate filter pass.
     *
    ...
    function applyUniqVal(value, table, colIndex) {
        ...
        // Write the value directly (no decorative prefix while not focused)
        input.value = value;

        // Apply the "active" background so the field looks highlighted
        const activeBg = Lib.settings.sa_col_filter_active_bg || '#fff9c4';
        input.style.backgroundColor = activeBg;

        // Dispatch a synthetic input event so the debounced filter fires
    ...
    function makeTableSortableUnified(table, sortKey) {
        ...
        headers.forEach((th, index) => {
            ...
            // 📊 button — floats right; must be the FIRST child appended so
            // CSS float:right places it at the trailing edge of the cell.
            const uniqBtn = document.createElement('span');
            uniqBtn.className = 'mb-col-uniq-btn';
            uniqBtn.textContent = '📊';
            uniqBtn.title = 'Show unique column values (click or Enter to apply as filter)';
            uniqBtn.setAttribute('tabindex', '0');
            uniqBtn.setAttribute('role', 'button');
            uniqBtn.setAttribute('aria-label', `Unique values — ${colName}`);
            uniqBtn.addEventListener('click', (ev) => {
                ev.preventDefault();
                ev.stopPropagation();
                openUniqDrop(uniqBtn, table, index);
            });
            // Space / Enter open the panel; other keys forwarded to the open panel's handler
            ...
            // Float-right icon must be inserted BEFORE inline content
