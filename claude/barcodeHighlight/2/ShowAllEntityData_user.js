...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
     ...
     * The button is the same height as #mb-visible-btn (inherits height from
     * uiActionBtnBaseCSS) but narrower — only horizontal padding is reduced so
     * the button takes less horizontal space without affecting the bar's vertical
     * rhythm.
     *
     * Visibility guard: the button is only injected when a "Barcode" <th> header
     * or at least one td.barcode-cell is present in the rendered table.  On page
     * types that have no Barcode column (events, works, recordings, …) the function
     * returns silently so no orphan button appears in the controls bar.
     *
    ...
    function addBarcodeHighlightToggleButton() {
        ...
        //   1. A <th> with the exact text "Barcode" in any table.tbl header row — present
        //      even when no release carries a barcode value (empty cells).
        //   2. A td.barcode-cell written by barcodeProcessTable (or by chaban's script) —
        //      fallback for tables where the header text differs from the canonical "Barcode".
        // If neither is found the button is silently suppressed; no warn log is emitted
        ...
        btn.style.cssText = uiActionBtnBaseCSS()
            ...
            .replace(/padding:(\S+)\s+(\S+);/, (_, v, h) => `padding:${v} 4px;`);
