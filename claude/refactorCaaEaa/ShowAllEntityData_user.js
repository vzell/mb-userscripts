...
(function() {
    ...
    const ColumnDataExtractor = {
         ...
         * from the Event cell.  Instead only the inner <bdi> element is **cloned**
         * into the synthetic cell — without the surrounding brackets — so the Cancelled
         * column shows only the bare word "cancelled", while the Event column continues
         * to display the full "(cancelled)" label alongside the event title.
         *
         * Sortability and dropdown filtering:
        ...
        cancelledEvent(sourceCell) {
            ...
            if (sourceCell) {
                ...
                if (cancelledSpan) {
                    // Clone only the inner <bdi> element so the Cancelled column shows
                    // the bare word "cancelled" without the surrounding "(" and ")"
                    // brackets that appear in the original span.  The full "(cancelled)"
                    // label remains untouched in the Event column.
                    const bdi = cancelledSpan.querySelector('bdi');
                    if (bdi) {
                        tdCancelled.appendChild(bdi.cloneNode(true));
                    } else {
                        // Fallback: no <bdi> present — extract text, strip brackets.
                        const text = cancelledSpan.textContent.replace(/[()]/g, '').trim();
                        tdCancelled.appendChild(document.createTextNode(text));
                    }
                    appendSortKey('yes');
                } else {
                    appendSortKey('no');
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function openUniqDrop(btn, table, colIndex) {
        ...
        const makeSynItem = (mode, label, count) => {
            ...
            // overridden by class-based rules).
        ...
        // Key handler on the quickfilter input
        const qfKH = (ev) => {
            if (!_uniqDropOwner) return;
            const items = allItems();
            if (ev.key === 'ArrowDown') {
                ev.preventDefault(); ev.stopPropagation();
                moveFocus(Math.min(focIdx + 1, items.length - 1));
            } else if (ev.key === 'ArrowUp') {
                ev.preventDefault(); ev.stopPropagation();
                moveFocus(Math.max(focIdx - 1, 0));
            } else if (ev.key === 'Home') {
                ev.preventDefault(); ev.stopPropagation();
                moveFocus(0);
            } else if (ev.key === 'End') {
        ...
        // Also keep the original btn-level handler for when focus is on the button
        const kh = (ev) => {
            if (!_uniqDropOwner) return;
