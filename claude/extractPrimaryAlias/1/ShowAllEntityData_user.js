            ...
            if (sourceCell) {
                ...
                if (events.length > 0) {
                    ...
                    events.forEach((ev) => {
                        ...
                        if (dateSpan) {
                            const li = document.createElement('li');
                            li.textContent = dateSpan.textContent.trim();
                            ulD.appendChild(li);
                        }
                    });
    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                Lib.debug(
                    'indices',
                    `Detected indices → mainColIdx=${mainColIdx} (${headerNames[mainColIdx] || 'N/A'}), extractors=[${extractorSummary}], erasers=[${eraserSummary}], renderMultiRow=[${multiRowSummary}], excluded=[${indicesToExclude.join(',')}] for pageType: ${pageType}`
                );
