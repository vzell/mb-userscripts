...
(function() {
    ...
    const configSchema = {
        ...
        sa_expand_show_all_cells: {
            ...
            type: "checkbox",
            default: true,
            description: 'On pages such as "Place-Events" and "Artist-Works", automatically expand every "(show N more)" toggle inside data cells (li.show-all) before extracting rows. ' +
                         'For the live current page this dispatches a real click event and waits up to 5 s for the React component to finish rendering the extra items (li.show-less sentinel). ' +
                         'For server-fetched (DOMParser) pages it attempts an inline DOM reveal of any inline-hidden sibling <li> elements. ' +
                         'After expansion the accompanying "(show less)" node (li.show-less) is always removed from the final rendered output.'
        },
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                // Controlled by sa_expand_show_all_cells.  Runs for every page (live or
                // DOMParser) and expands li.show-all toggles in data cells (e.g. the
                // "Artists" column on Place-Events pages or "Recording artists" /
                // "Attributes" on Artist-Works pages) so the full content is available
                // to the row-extraction pipeline below.  li.show-less sentinels are
                // removed after each successful expansion.
                await expandShowAllCells(doc, p);
     ...
     *
     * Two expansion modes are used depending on the source of `doc`:
     *
     ...
     *     MAX_EXPAND_WAIT_MS ms), which MusicBrainz renders as a sentinel once the
     *     React component has finished inserting the extra items.  Every `li.show-less`
     *     node is removed from the live DOM after confirmation.
     *
     *   FETCHED document  (DOMParser — no JavaScript execution)
     *     Click events are no-ops in detached documents.  The function instead inspects
     *     every sibling `<li>` in the same `<ul>` for inline `display:none`,
     *     `visibility:hidden`, or the `hidden` attribute, and clears those constraints.
     *     Both `li.show-all` and `li.show-less` are removed afterwards.
     *
     * Controlled by the `sa_expand_show_all_cells` configuration flag.
     *
     * @param {Document} doc     - The document to process (live or DOMParser).
     * @param {number}   pageNum - 1-based page number for log context.
     * @returns {Promise<void>}
     */
    async function expandShowAllCells(doc, pageNum) {
        // ── Feature gate ────────────────────────────────────────────────────────────
        ...
        const MAX_EXPAND_WAIT_MS = 5000; // max ms to wait per show-all click on a live page
        ...
        const isLiveDoc = (doc === document);
        Lib.debug(
            'expand',
            `Page ${pageNum}: Found ${showAllItems.length} li.show-all element(s). ` +
            `Expansion mode: ${isLiveDoc ? 'LIVE (click + wait for li.show-less)' : 'FETCHED (inline DOM reveal)'}.`
        );
        ...
        for (let i = 0; i < showAllItems.length; i++) {
            ...
            if (isLiveDoc) {
                // ── LIVE document: dispatch click and wait for li.show-less ──────────
                Lib.debug('expand', `Page ${pageNum}: [${i}] Dispatching click event on show-all link: "${linkText}".`);
                link.click();

                const appeared = await pollForShowLess(
                    parentUl || document.body,
                    'li.show-less',
                    MAX_EXPAND_WAIT_MS,
                    POLL_INTERVAL_MS
                );
                ...
                if (appeared) {
                    const showLessNodes = (parentUl || document.body).querySelectorAll('li.show-less');
                    Lib.debug(
                        'expand',
                    ...
                    Lib.warn(
                        ...
                        `after clicking "${linkText}". Expansion may be incomplete.`
                    );
                ...
                // ── FETCHED document (DOMParser): inline DOM reveal ──────────────────
                // JavaScript is never executed in detached DOMParser documents, so click
                // events are silently ignored.  Attempt to reveal hidden siblings instead.
                Lib.debug(
                    'expand',
                    `Page ${pageNum}: [${i}] Fetched doc — attempting inline reveal for "${linkText}".`
                );
                ...
                if (parentUl) {
                    ...
                    allLis.forEach(li => {
                        // Never touch the structural show-all / show-less toggles themselves
                        if (li.classList.contains('show-all') || li.classList.contains('show-less')) return;

                        const st = li.style;
                        if (st.display === 'none') {
                            Lib.debug('expand', `Page ${pageNum}: [${i}]   Revealing li (had display:none): "${li.textContent.slice(0, 60)}"`);
                            st.display = '';
                            revealedCount++;
                        }
                        if (st.visibility === 'hidden') {
                            Lib.debug('expand', `Page ${pageNum}: [${i}]   Revealing li (had visibility:hidden): "${li.textContent.slice(0, 60)}"`);
                            st.visibility = '';
                            revealedCount++;
                        }
                        if (li.hasAttribute('hidden')) {
                            Lib.debug('expand', `Page ${pageNum}: [${i}]   Revealing li (had [hidden]): "${li.textContent.slice(0, 60)}"`);
                            li.removeAttribute('hidden');
                            revealedCount++;
                        }
                    });

                    // Always remove both show-all and show-less from the list regardless of how many items
                    // were revealed, so they never end up in the final extracted rows.
                    const togglesRemoved = Array.from(
                        parentUl.querySelectorAll('li.show-all, li.show-less')
                    );
                    togglesRemoved.forEach(el => {
                        Lib.debug('expand', `Page ${pageNum}: [${i}]   Removing toggle li: "${el.textContent.slice(0, 80)}"`);
                        el.remove();
                    });

                    Lib.debug(
                        'expand',
                        `Page ${pageNum}: [${i}] Inline reveal complete — revealed ${revealedCount} hidden li(s), ` +
                        `removed ${togglesRemoved.length} toggle node(s) (li.show-all + li.show-less).`
                    );
                    expandedCount++;

                } else {
                    // No parent <ul> — remove the orphan show-all node and warn
                    Lib.warn(
                        'expand',
                        `Page ${pageNum}: [${i}] No parent <ul> found for li.show-all "${linkText}" — ` +
                        `cannot perform inline reveal. Removing the show-all node only.`
                    );
                    showAllLi.remove();
                    skippedCount++;
                }
