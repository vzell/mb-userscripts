...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
     ...
     * Supports cross-tag matches by building a virtual concatenated string
     * per cell and mapping regex match positions back to individual text nodes.
     *
     ...
     * @param {number}             targetColIndex   - Column index to highlight (-1 = all columns)
     * @param {boolean}            isRegExp         - Whether to treat query as a regular expression
     * @param {string}             highlightType    - 'auto' | 'prefilter' | 'global' | 'column'
     */
    function highlightText(row, query, isCaseSensitive, targetColIndex = -1, isRegExp = false, highlightType = 'auto') {
        ...
        row.querySelectorAll('td').forEach((td, idx) => {
            ...
            _highlightCellCrossTag(td, regex, className);
        });
    }

    /**
     * Highlights all regex matches inside a single table cell, including
     * matches whose text spans across multiple sibling tags / text nodes.
     *
     * Algorithm:
     *   1. Collect every text node in the cell (depth-first) together with its
     *      start-offset inside a virtual concatenated string.
     *   2. Run the regex once against that virtual string to find all matches.
     *   3. For each text node that overlaps one or more matches, replace it with
     *      a DocumentFragment whose pieces are either plain Text nodes or
     *      <span class="…"> highlight wrappers.
     *
     * Nodes are replaced back-to-front (by reverse segment index) so earlier
     * siblings' positions in the live DOM are not disturbed while iterating.
     *
     * @param {HTMLElement} cell      - The <td> element to process
     * @param {RegExp}      regex     - Pre-compiled, global-flagged regex
     * @param {string}      className - CSS class to apply to highlight spans
     */
    function _highlightCellCrossTag(cell, regex, className) {
        // ── Step 1: collect text nodes + virtual-string offsets ───────────────────
        const nodeFilter = {
            acceptNode(node) {
        ...
        const walker = document.createTreeWalker(cell, NodeFilter.SHOW_TEXT, nodeFilter, false);
        ...
        const virtualStr = segments.map(s => s.node.nodeValue).join('');

        // ── Step 2: find all matches in the virtual string ────────────────────────
        regex.lastIndex = 0;
        const matches = [];
        let m;
        while ((m = regex.exec(virtualStr)) !== null) {
        ...
        // ── Step 3: replace text nodes back-to-front ──────────────────────────────
        for (let si = segments.length - 1; si >= 0; si--) {
            const { node: textNode, start: nodeStart, end: nodeEnd } = segments[si];
            const nodeText = textNode.nodeValue;

            // Find matches that overlap this node's range in the virtual string
            const overlapping = matches.filter(m => m.start < nodeEnd && m.end > nodeStart);
            if (overlapping.length === 0) continue;

            // Build a flat list of {text, highlight} fragments for this node
            const fragments = [];
            let pos = 0; // cursor inside nodeText

            for (const match of overlapping) {
                const localStart = Math.max(0, match.start - nodeStart);
                const localEnd   = Math.min(nodeText.length, match.end - nodeStart);

                if (pos < localStart) {
                    fragments.push({ text: nodeText.slice(pos, localStart), highlight: false });
                }
                if (localStart < localEnd) {
                    fragments.push({ text: nodeText.slice(localStart, localEnd), highlight: true });
                }
                pos = localEnd;
            }
            if (pos < nodeText.length) {
                fragments.push({ text: nodeText.slice(pos), highlight: false });
            }

            // Build a DocumentFragment and replace the original text node
    ...
    function getUniqDropEl() {
        ...
        // Close on Escape pressed anywhere
        document.addEventListener('keydown', (ev) => {
            if (ev.key === 'Escape' && _uniqDropOwner) closeUniqDrop();
        }, true);
    ...
    function openUniqDrop(btn, table, colIndex) {
        ...
        if (vals.length === 0) {
            ...
            vals.forEach(v => {
                ...
                item.textContent = v;
                item.title = v;
                // preventDefault prevents the click from blurring a focused filter input
        ...
        // ---- Keyboard navigation ------------------------------------------
        // All key events are caught on `btn` so the panel never needs focus.
        // moveFocus() manually adjusts drop.scrollTop instead of using
        ...
        const kh = (ev) => {
            ...
            if (ev.key === 'ArrowDown') {
                ...
                if (focIdx >= 0 && items[focIdx]) {
                    applyUniqVal(items[focIdx].textContent, table, colIndex);
                    closeUniqDrop();
                }
            } else if (ev.key.length === 1) {
                // Jump to first item starting with this character
                const ch = ev.key.toLowerCase();
                const ti = items.findIndex(el =>
                    el.textContent.toLowerCase().startsWith(ch)
                );
                if (ti !== -1) moveFocus(ti);
            }
        };
        ...
        const dropH = Math.min(320, vals.length * 29 + 12);
