...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
     ...
     *
     * Algorithm overview:
     *   1. Walk every text node under `root` (excluding <script>/<style>), recording
     *      each node's absolute character offset within the cell's concatenated plain text.
     *   2. Run `regex` against the concatenated text to find all match ranges.
     *   3. For each text node that overlaps one or more matches, compute the segment
     *      boundaries (text node start/end + clamped match start/end positions), then
     *      replace the original text node with a DocumentFragment whose segments are
     *      either plain Text nodes (outside all matches) or <span class={className}>
     *      elements (inside a match).
     *   4. Text nodes are processed in reverse document order so that prior nodes'
