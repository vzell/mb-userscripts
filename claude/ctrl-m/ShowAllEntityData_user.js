...
(function() {
    ...
    const configSchema = {
        ...
        sa_keyboard_shortcut_prefix: {
            ...
            default: "Ctrl+.",
            description: "Keyboard shortcut prefix key combination (expects a second key press to be complete, e.g. Ctrl+., Ctrl+M, Alt+X, Ctrl+Shift+,)"
        },
    ...
    function parsePrefixShortcut(str) {
        const parts = (str || 'Ctrl+.').trim().split('+');
    ...
    /**
     * Returns the display string for the configured prefix shortcut (e.g. "Ctrl+.").
     * Falls back to "Ctrl+." when the setting is not yet available.
     * @returns {string}
     */
    ...
    /**
     * Displays a floating tooltip listing all Ctrl+. prefix-mode shortcuts.
    ...
    /**
     * Removes the Ctrl+. prefix-mode tooltip from the DOM if it is currently visible.
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function makeTableSortableUnified(table, sortKey) {
        ...
        headers.forEach((th, index) => {
            ...
            // Float-right icon first, then the rest of the header content
     ...
     * Used as the Ctrl+. + "e" prefix-mode shortcut target.
     */
     ...
     * Used as the Ctrl+. + "v" prefix-mode shortcut target.
     */
     ...
     * Used as the Ctrl+. + "d" prefix-mode shortcut target.
     */
