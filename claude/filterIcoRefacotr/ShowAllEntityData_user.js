...
(function() {
    ...
    function clearAllFilters() {
        // Clear global filter — strip any focus prefix and reset the field.
        const filterInput = document.getElementById('mb-global-filter-input');
        if (filterInput) {
            filterInput.value = '';
            // Reset the placeholder to idle state (no focus prefix) in case the
            // field currently has focus and is showing the prefix placeholder.
            const _pfxNow = getFilterFocusPrefix();
            const _gfIdle = (activeDefinition && activeDefinition.tableMode === 'multi')
                ? 'Global Filter… works across all sub-tables'
                : 'Global Filter…';
            filterInput.placeholder = (document.activeElement === filterInput)
                ? _pfxNow + _gfIdle
                : _gfIdle;
        }
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    // Base placeholder (without the focus prefix) — the focus handler prepends
    // the prefix dynamically when the field gains keyboard focus.
    ...
    // ── Global filter: decorative focus prefix (mirrors column filter behaviour) ────────
    // The base placeholder text varies by table mode; capture it once so focus/blur can
    // restore it correctly without re-querying activeDefinition every time.
    const _gfBasePlaceholder = activeDefinition && activeDefinition.tableMode === 'multi'
        ? 'Global Filter… works across all sub-tables'
        : 'Global Filter…';

    filterInput.addEventListener('focus', () => applyFilterFocusStyle(
        filterInput,
        _gfBasePlaceholder
        // No focusBg argument — global filter has no persistent focus-background tint
    ));

    filterInput.addEventListener('blur', () => {
        // hasActiveBg=false: global filter uses border colour (not background) to signal
        // active state, so we never apply a persistent background after blur.
        removeFilterFocusStyle(filterInput, _gfBasePlaceholder, false);
        // After stripping the prefix the stored value is the clean query; re-run the
        // filter so runFilter() reads the clean value (important if blur fires while a
        // filter is still active, e.g. when the user clicks away without pressing Enter).
        runFilter();
    });

    filterInput.addEventListener('keydown', (e) => guardFilterPrefixKeydown(filterInput, e));

    // Use debounced version for input events; prepend prefix on first keystroke into
    // a focused-but-empty field (same pattern as column filter inputs).
    filterInput.addEventListener('input', (e) => {
        const prefix = getFilterFocusPrefix();
        if (filterInput.value !== '' && !filterInput.value.startsWith(prefix)) {
    ...
    filterClear.addEventListener('click', () => {
        // Strip any prefix and clear the field, then reset the placeholder to
        // the focus-state version so the user can see the hint immediately.
        filterInput.value = '';
        const prefix = getFilterFocusPrefix();
        filterInput.placeholder = prefix + _gfBasePlaceholder;
        runFilter();
    });
