...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    // On release-group pages the h1 may contain a trailing
    //   <span class="comment">( <bdi><i title="Primary alias">…</i></bdi> )</span>
    // which renders the alias text — including surrounding parentheses — on the
    // same line as the entity title and our action buttons.
    //
    // We remove that span from the h1 and inject a lightweight replacement line
    // directly above <p class="subheader"> (or above the h1's next sibling when
    // no subheader exists).  The replacement reads:
    //   «<title-value>: <alias-text>»
    // where <title-value> is taken from the <i title="…"> attribute (e.g.
    // "Primary alias") and <alias-text> is the element's text content.
    //
    // The injected element uses the id "mb-h1-alias-line" so it can be found /
    // removed deterministically if this function is ever called again on a
    // soft-navigation reload.
    /**
     * Relocates the `<span class="comment">` disambiguation block that MusicBrainz
     * sometimes injects into the h1 on release-group pages.
     *
     * The span is removed from the h1 and a new `<p id="mb-h1-alias-line">` is
     * inserted immediately before `<p class="subheader">` (or, when no subheader
     * is found, immediately after the h1 element itself).  The rendered text
     * follows the pattern «<i-title>: <alias-text>» — e.g.
     * "Primary alias: Bruce Springsteen & The E Street Band at Athletic &
     *  Convocation Center".
     *
     * @returns {void}
     */
    (function relocateH1CommentSpan() {
        ...
        // Find the comment span inside the h1.
        const commentSpan = h1El.querySelector(':scope > span.comment');
        if (!commentSpan) return; // Nothing to do on pages without this span.

        // Extract the <i> element that carries title="Primary alias" (or similar).
        const iEl = commentSpan.querySelector('i[title]');
        const aliasTitle = iEl ? iEl.getAttribute('title').trim() : '';
        const aliasText  = iEl ? iEl.textContent.trim()
                                : commentSpan.textContent.replace(/^\s*\(\s*|\s*\)\s*$/g, '').trim();

        if (!aliasText) {
            // Edge case: empty alias — leave the span in place rather than showing nothing.
            return;
        }

        // Remove the span from the h1 so it no longer appears on the title line.
        commentSpan.remove();

        // Build the replacement paragraph.
        const aliasLine = document.createElement('p');
        aliasLine.id = 'mb-h1-alias-line';
        // Inherit a modest, italicised appearance close to the MB subheader style
        // without overriding the page's own stylesheet cascade.
        ...
        // Insert before <p class="subheader"> when it exists; otherwise right
        // after the h1 so we preserve DOM reading order.
        ...
        Lib.debug('init', 'Relocated h1 comment span → #mb-h1-alias-line', { aliasTitle, aliasText });
    })();
