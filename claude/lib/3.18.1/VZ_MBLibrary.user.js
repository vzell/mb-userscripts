...
const LIBRARY_VERSION = '3.18.1+2026-04-01';
...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        function _openTableEditor(key, cfg) {
            ...
            // Escape key: close only the table editor, not the parent settings dialog.
            // Registered in the capture phase so it fires before settingsEscHandler and
            // stopImmediatePropagation() prevents the parent handler from also closing.
            // The handler is removed when the panel is removed from the DOM.
            const _tblEscHandler = (e) => {
                if (e.key !== 'Escape') return;
                const _panel = document.getElementById(_pid);
                if (!_panel) {
                    // Panel already gone — clean up stale listener
                    document.removeEventListener('keydown', _tblEscHandler, true);
                    return;
                }
                // Only intercept when focus is inside the table-editor panel.
                // If focus is elsewhere (e.g. in the parent settings dialog search
                // field), let the event propagate so other handlers can act on it.
            ...
            document.addEventListener('keydown', _tblEscHandler, true);

            // Also clean up the Escape handler when the panel is removed via the
            // close button or the toggle (second click removes the panel directly).
            const _tblObserver = new MutationObserver(() => {
                if (!document.getElementById(_pid)) {
                    document.removeEventListener('keydown', _tblEscHandler, true);
                    _tblObserver.disconnect();
                }
            });
            _tblObserver.observe(document.body, { childList: true, subtree: false });
        }
