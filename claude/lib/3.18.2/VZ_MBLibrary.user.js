...
const LIBRARY_VERSION = '3.18.2+2026-04-01';
    ...
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        function _openTableEditor(key, cfg) {
            ...
            panel.addEventListener('keydown', (e) => {
                if (e.key !== 'Escape') return;
                e.stopPropagation();   // prevent settingsEscHandler from also firing
                e.preventDefault();
                if (document.activeElement !== panel &&
                    panel.contains(document.activeElement)) {
                    // Step 1: blur the focused input, hand focus back to the panel
                    document.activeElement.blur();
                    panel.focus();
                } else {
                    // Step 2: close the panel (same as footer Close button)
