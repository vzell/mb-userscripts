...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function addBarcodeHighlightToggleButton() {
        ...
        // Intentionally smaller than standard action buttons — the barcode glyph
        // is decorative and needs less visual weight in the controls bar.
        btn.style.cssText = uiActionBtnBaseCSS()
            .replace(/font-size:[^;]+;/, 'font-size:0.65em;')
            .replace(/padding:[^;]+;/, 'padding:1px 5px;')
            .replace(/height:[^;]+;/, 'height:20px;');
