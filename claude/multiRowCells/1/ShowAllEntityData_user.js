...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
     ...
     *     item remains visible; a ▶ toggle icon is appended at the END of the
     *     first `<li>` (inline, not below the list).
     ...
     *     the 📊 button.  The header button is a true toggle:
     *       ▶ (all/some collapsed) → click → drives ALL cell toggles to ◀
     *       ◀ (all expanded)       → click → drives ALL cell toggles to ▶
     *     The icon updates to reflect the resulting state.
     *   - A single global toggle button (`id="mb-col-collapse-all-btn"`) covers
     *     all collapsable columns simultaneously.  Its icon (▶/◀) and label text
     *     update to reflect the resulting state.  The global button drives every
     *     column-header button to the same target state regardless of their
     *     current individual states (always consistent).
     *
    ...
    function initCollapsableColumns(table) {
        ...
        collapsableColumns.forEach(colName => {
            ...
            // The cell toggle is appended to the END of the first <li> so it
            // sits inline after the first item's text — not on a new line below
            // the entire list.
            multiRowCells.forEach(({ td, lis }) => {
                ...
                // ── Place toggle at the very end of the FIRST <li> (inline) ──
                lis[0].appendChild(cellToggle);
            });

            // ── Add true-toggle button to column header ───────────────────────
            // State: ▶ = all cells currently collapsed (click to expand all)
            //        ◀ = all cells currently expanded  (click to collapse all)
            // After the action all cell toggles are driven to the target state,
            // then the header icon is updated to match the new state.
            ...
            // Initial state: collapsed (▶ = click to expand)
            collapseHdrBtn._expanded = false;
            ...
            collapseHdrBtn.addEventListener('click', (ev) => {
                ev.stopPropagation();
                // Determine target: expand if currently collapsed, collapse if expanded.
                const targetExpand = !collapseHdrBtn._expanded;
                const targetIcon   = targetExpand ? '◀' : '▶';

                // Drive every cell toggle in this column to the target state,
                // clicking only those that are not already in the target state.
                ...
                // Update header button state and icon.
                collapseHdrBtn._expanded = targetExpand;
                collapseHdrBtn.textContent = targetIcon;
                ...
                collapseHdrBtn.setAttribute(
                    'aria-label',
                    targetExpand
                        ? `Collapse all cells in column: ${colName}`
                        : `Expand all collapsed cells in column: ${colName}`
                );
            });
            ...
            if (hdrFlex) {
                ...
                if (uniqBtn) {
                    ...
                    // edge of the flex container, with the toggle immediately
                    // before 📊.
        ...
        // ── Wire global true-toggle button ────────────────────────────────────
        // State: ▶ = all columns currently collapsed (click to expand all)
        //        ◀ = all columns currently expanded  (click to collapse all)
        // Each click drives ALL column-header buttons to the same target state,
        // then updates icon and label text to reflect the new state.
        const globalBtn = document.getElementById('mb-col-collapse-all-btn');
        if (globalBtn) {
            if (anyColumnHasMultiRow) {
                globalBtn.style.display = '';
                // Reset global toggle state on every (re-)init.
                globalBtn._expanded = false;
                globalBtn.textContent = '▶ Expand columns';

                // Re-wire onclick each call (safe for disk-load re-runs).
                globalBtn.onclick = () => {
                    const targetExpand = !globalBtn._expanded;

                    // Drive every column-header button to the target state.
                    // We call the button's click handler only when it is not
                    // already in the desired state, so each column ends up in a
                    // uniform, predictable state after the global action.
                    collapseHdrBtns.forEach(btn => {
                        const btnExpanded = btn._expanded;
                        if (btnExpanded !== targetExpand) {
                            btn.click();
                        }
                    });

                    // Update global button icon and label.
                    globalBtn._expanded = targetExpand;
