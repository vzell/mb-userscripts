...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
     ...
     * based on whether any multi-row data cell in `table` — collapsed OR
     * expanded — currently contains a filter-highlight mark.
     *
    ...
    function updateGlobalCollapseButtonHighlight(table) {
        ...
        const hasAnyHighlight = !!table.querySelector(
    ...
    function ensureCollapseDelegate(table) {
        ...
        tbody.addEventListener('click', (ev) => {
            ...
            if (tbl) updateGlobalCollapseButtonHighlight(tbl);
        });
    ...
    function initCollapsableColumns(table) {
        ...
        // Drives ALL column-header buttons together.  State is read from the
        // button's own textContent so it stays correct across re-inits.
        const globalBtn = document.getElementById('mb-col-collapse-all-btn');
        if (globalBtn) {
        ...
        // shown/hidden by the anyColumnHasMultiRow check.
