    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                Lib.debug(
                    'indices',
                    `Detected indices → mainColIdx=${mainColIdx} (${headerNames[mainColIdx] || 'N/A'}), extractors=[${extractorSummary}], erasers=[${eraserSummary}], excluded=[${indicesToExclude.join(',')}] for pageType: ${pageType}`
                );
