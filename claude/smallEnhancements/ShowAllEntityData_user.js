...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function saveTableDataToDisk() {
        ...
        try {
            ...
            // For multi-button pages (e.g. "Official RGs" / "Non-official RGs") append the
            // clicked button label as a slug so saved files are immediately distinguishable.
            // For query-string pages (link_type_id=…) append the first visible h3 header
            // text as a slug for the same reason.
            const timestamp = new Date().toISOString().replace(/[:.]/g, '-').slice(0, -5);
            let _filenameSuffix = '';
            if (activeDefinition.buttons && activeDefinition.buttons.length > 1 && activeDefinition.label) {
                // Multi-button page: use the clicked button's label (lowercased, spaces → underscores,
                // all non-alphanumeric characters stripped).
                _filenameSuffix = '-' + activeDefinition.label
                    .toLowerCase()
                    .replace(/\s+/g, '_')
                    .replace(/[^a-z0-9_]/g, '');
            } else if (isFilteredRelationshipPage) {
                // Query-string (link_type_id) page: derive slug from the first h3 header text
                // rendered by MusicBrainz (e.g. "performed Guitar on" → "performed_guitar_on").
                const firstH3 = document.querySelector('#content h3');
                if (firstH3) {
                    _filenameSuffix = '-' + firstH3.textContent.trim()
                        .toLowerCase()
                        .replace(/\s+/g, '_')
                        .replace(/[^a-z0-9_]/g, '');
                }
            }
            const filename = `mb-${pageType}${_filenameSuffix}-${timestamp}.json.gz`;
