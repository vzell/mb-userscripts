...
.sa-unicode-row-close .sa-unicode-cell-char {
     ...
     *
     * If the span has no wrapping anchor the call is a no-op — this covers "Path B"
     * bare spans produced by the SA `caa` extractor on artist-releasegroups pages
     * when the jesus2099 anchor has already been stripped by a columnEraser.
     *
     ...
     * the API returns images, updates the anchor's `title` attribute with the
     * image count in the form "N image(s) found in this release".
     *
     ...
     * errors or non-200 responses the anchor's title is left unchanged and the
    ...
    async function caaEnrichReleaseGroupIcon(anchor) {
        ...
        try {
            ...
            if (count > 0) {
                anchor.title = count + ' image' + (count !== 1 ? 's' : '') + ' found in this release';
     ...
     * bare spans (no ancestor anchor) are silently skipped by caaLoadIcon.
     *
     * @param {HTMLTableElement} table
     */
    ...
    /**
     * Returns true when at least one row in `table`'s tbody has real cover art.
     *
     * Detection strategy (in priority order):
     *
     * 1. **Synthetic CAA/EAA column present** — checks for any
     *    `span.mb-caa-sort-key` whose trimmed text is "yes".  These spans are
     *    injected by the `caa` ColumnDataExtractor at fetch time using MB's own
     *    artwork-icon indicator, making them the most reliable source of truth.
     *
     * 2. **No CAA/EAA column** — falls back to checking whether at least one
     *    `mb-caa-inline-val` span with text "yes" exists (set by `initCaaInlinePics`
     *    after a successful image load).  When neither column nor inline-val spans
     *    have been stamped yet, returns false conservatively.
     *
     * Used by `initCaaPics()` to suppress the per-table toggle button when no row
     * in the table has actual artwork — avoids an empty button that would open a
     * blank bigbox strip.
     *
     * @param  {HTMLTableElement} table
     * @returns {boolean}
     */
    function caaTableHasAnyArtwork(table) {
        // Strategy 1: synthetic CAA/EAA column with mb-caa-sort-key "yes" spans
        if (caaFindCaaColumnIndex(table) !== -1) {
            const keys = table.querySelectorAll('tbody span.mb-caa-sort-key');
            return Array.from(keys).some(k => k.textContent.trim() === 'yes');
        }
        // Strategy 2: inline-val spans already stamped by initCaaInlinePics
        const inlineVals = table.querySelectorAll('tbody span.mb-caa-inline-val');
        if (inlineVals.length > 0) {
            return Array.from(inlineVals).some(k => k.textContent.trim() === 'yes');
        }
        // No authoritative indicator yet — conservatively allow the button
        // (caaCountLinks == 0 guard in caaCreateOrUpdateToggleButton still applies)
        return caaCountLinks(table).count > 0;
    }
    ...
    function initCaaPics() {
        ...
        tables.forEach((table, i) => {
            ...
            // Only show the toggle button when at least one row actually has artwork.
            // caaTableHasAnyArtwork checks mb-caa-sort-key "yes" spans (synthetic CAA
            // column, most reliable) or mb-caa-inline-val "yes" spans (inline feature)
            // and falls back to a link-count guard when neither is available yet.
            const hasArt = caaTableHasAnyArtwork(table);
            const btnId  = hasArt ? caaCreateOrUpdateToggleButton(table, i, count, firstImgUrl) : undefined;
            caaInitBigPics(table, i, btnId); // btnId may be undefined if no button
            processed++;
        });
    ...
    function initCaaInlinePics() {
        ...
        tables.forEach(table => {
            ...
            table.querySelectorAll('tbody tr').forEach(tr => {
                ...
                // ── Sort / filter key span ────────────────────────────────────
                // Mirrors the mb-caa-sort-key pattern used by the synthetic CAA
                // column extractor.  The span is hidden from the browser but IS
                // read by getCleanColumnText() / getCleanVisibleText() because
                // their TreeWalker does not skip display:none elements and the
                // span's class is not in the artwork-icon/caa-icon filter list.
                //
                // Effect:
                //   • The unique-values dropdown for this column shows "…title yes"
                //     and "…title no" entries; the quickfilter lets users type "yes"
                //     to see only rows with artwork or "no" for rows without.
                //   • The column text-filter box also accepts "yes"/"no" to match.
                //   • Sorting by this column places "no" rows before "yes" rows
                //     (alphabetical ascending).
                //
                // Initial value is "no"; updated to "yes" on successful img load.
                // Stays "no" on network error (artwork genuinely absent or 404).
                const sortKey = document.createElement('span');
                sortKey.className    = 'mb-caa-inline-val';
                sortKey.setAttribute('aria-hidden', 'true');
                sortKey.style.display = 'none';
                sortKey.textContent  = 'no';
                ph.appendChild(sortKey);

                img.addEventListener('load', function() {
                    this.style.display = 'inline';
                    // Promote the sort/filter key to "yes" once the image confirms
                    // that artwork actually exists on the Cover Art Archive.
                    const sk = this.parentNode.querySelector('.mb-caa-inline-val');
                    if (sk) sk.textContent = 'yes';
                });
                // On error: placeholder stays as an invisible spacer — alignment
                // preserved; sort-key remains "no" (correct: no artwork available).
