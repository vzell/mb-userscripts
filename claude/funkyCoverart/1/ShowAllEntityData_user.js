...
.sa-unicode-row-close .sa-unicode-cell-char {
    ...
    /**
     * Loads thumbnails into all caa/eaa/artwork-icon spans in `table`.
     * Only spans inside `<a href="…/cover-art">` anchors are processed;
     * bare spans (no ancestor anchor) are silently skipped.
     *
     * Mirrors the "SMALL PICS" section of jesus2099's main loop and his
     * `caaIcons` loop that calls `loadCaaIcon()`.
     *
     * @param {HTMLTableElement} table
     */
    function caaInitSmallPics(table) {
        ...
        icons.forEach(icon => caaLoadIcon(icon));
