...
(function() {
    ...
    /**
     * Removes configured marker spans from a data row's cells in-place.
     *
     * For each eraser descriptor whose column index has been resolved (`colIdx !== -1`),
     * all <span> children of the target cell are inspected.  A span is removed when its
     * trimmed text content matches one of the eraser symbols declared in `entry.erasers`.
     *
    ...
    function applyColumnErasers(row, eraserDescriptors) {
        ...
        for (const entry of eraserDescriptors) {
            ...
            cell.querySelectorAll('span').forEach(span => {
                ...
                if (entry.erasers.includes(symbol)) {
                    span.remove();
                    removedCount++;
            ...
            if (removedCount === 0) {
                Lib.debug('extract', `columnEraser: no matching spans found in column "${entry.sourceColumn}" (colIdx=${entry.colIdx}), erasers=[${entry.erasers.join(', ')}]`);
            }
        }
    }
    ...
    const pageDefinitions = [
        ...
        },
        {
            ...
            features: {
                columnErasers: [
                    { sourceColumn: 'Release groups', erasers: ['▶', '➕'] }
                ],
                columnExtractors: [
        ...
        reader.onload = async (e) => {
            try {
                ...
                // colouring all action buttons when no label was saved or none matched.
                {
                    ...
                    if (savedLabel) {
                        allActionButtons.forEach(btn => {
                            if (btn.textContent.trim().toLowerCase() === savedLabel) {
                                btn.style.backgroundColor = '#ccffcc';
                                btn.style.color = 'black';
                                matched = true;
                                Lib.debug('cache', `Disk-load: coloured matching action button green: "${btn.textContent.trim()}"`);
                            }
                        });
                    }
                    if (!matched) {
