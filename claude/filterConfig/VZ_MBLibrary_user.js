...
const LIBRARY_VERSION = '3.15.0+2026-03-27';
...
const VZ_MBLibrary = (function() {
    return function(scriptId, scriptName, configSchema = null, changelog = null, debugEnabled = true, remoteConfig = null) {
        ...
        const settingsInterface = {
             ...
             *   Optional map of function-name → callable.  Entries with
             *   `type: 'function'` in configSchema look up their `default`
             *   key here at click time.  The consumer script populates this
             *   by passing the registry to Lib.showSettings(registry).
             *   Example: { 'openEditPinnedFilterList': () => myFn() }
             */
            showModal: function(functionRegistry = {}) {
                ...
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    ...
                    if (isCheck) {
                        ...
                        // Function-type entries render as a single action button.
                        // No input element is needed — clicking calls the registered
                        // function directly.  The button label is the cfg.label text.
                        // The hidden input is only present so that takeSnapshot /
                        // hasUnsavedChanges / save can safely skip this entry via the
                        // 'function' type guard without needing a special-case element.
                        inputHtml = `<button type="button" id="${inputId}-fn-btn" `
                            + `style="font-size:0.85em;padding:4px 12px;cursor:pointer;`
                            + `border:1px solid #90a4ae;border-radius:5px;`
                            + `background:#e8f0fe;color:#1a237e;font-weight:600;`
                            + `vertical-align:middle;transition:background 0.15s,box-shadow 0.15s;" `
                            + `title="${escAttr(cfg.description || cfg.label)}">`
                            + `🔧 ${escAttr(cfg.label)}</button>`;
                    } else {
                    ...
                    row.dataset.vzDefault   = (isPopupDialog || isFunction) ? '' : String(cfg.default ?? '');
                    ...
                    col2.innerHTML = (isPopupDialog || isFunction) ? '<em>(action)</em>' : escAttr(String(cfg.default));
                ...
                // ── Function-type button wiring ───────────────────────────────────
                // Each entry with type:'function' gets a button whose onclick calls
                // the function registered under cfg.default in functionRegistry.
                // If the registry has no entry for that name a console warning is
                // emitted and nothing else happens (no crash, no UI change).
                Object.entries(configSchema).forEach(([key, cfg]) => {
                    if (cfg.type !== 'function') return;
                    const inputId = `${scriptId}-input-${key}`;
                    const fnBtn   = document.getElementById(`${inputId}-fn-btn`);
                    if (!fnBtn) return;

                    fnBtn.addEventListener('click', (e) => {
                        e.stopPropagation();
                        const fnName = cfg.default;
                        const fn     = functionRegistry[fnName];
                        if (typeof fn === 'function') {
                            fn();
                        } else {
                            console.warn(
                                `[VZ_MBLibrary] showModal: no function registered for key "${fnName}" ` +
                                `(setting: "${key}"). Pass it via Lib.showSettings({ "${fnName}": yourFn }).`
                            );
                        }
                    });

                    // Hover style
                    fnBtn.addEventListener('mouseenter', () => {
                        fnBtn.style.background  = '#c5cae9';
                        fnBtn.style.boxShadow   = '0 2px 6px rgba(0,0,0,0.18)';
                    });
                    fnBtn.addEventListener('mouseleave', () => {
                        fnBtn.style.background  = '#e8f0fe';
                        fnBtn.style.boxShadow   = '';
                    });
                });
                ...
                const takeSnapshot = () => {
                    const snap = {};
                    for (const key in configSchema) {
                        const t = configSchema[key].type;
                        if (t === 'divider' || t === 'function') continue;
                        const el = document.getElementById(`${scriptId}-input-${key}`);
                        if (!el) continue;
                        snap[key] = t === 'checkbox' ? el.checked : el.value;
                    }
                    return snap;
                };
                ...
                const hasUnsavedChanges = () => {
                    for (const key in configSchema) {
                        ...
                        const el = document.getElementById(`${scriptId}-input-${key}`);
                        if (!el) continue;
                        const current = t === 'checkbox' ? el.checked : el.value;
                ...
                document.getElementById(`${scriptId}-save-btn`).onclick = () => {
                    ...
                    for (const key in configSchema) {
                        ...
                        if (!input) continue;
                        newValues[key] = t === 'checkbox' ? input.checked : input.value;
                    }
                    this.save(newValues);
                    closeDialog();
                };
                ...
                document.getElementById(`${scriptId}-reset`).onclick = async () => {
                    ...
                    for (const key in configSchema) {
                        const _rtype = configSchema[key].type;
                        if (_rtype === 'divider' || _rtype === 'function') continue;
                        const inputId = `${scriptId}-input-${key}`;
                        const input   = document.getElementById(inputId);
                        if (!input) continue;
        ...
        return {
            ...
            showSettings:       (fnRegistry) => settingsInterface.showModal(fnRegistry || {}),
            libPrefs:           _libPrefs,
