...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function getColFilters(table, isCaseSensitive, isRegExp) {
        ...
        // Find the associated filter-status span so we can report regexp errors there.
        ...
        Array.from(table.querySelectorAll('.mb-col-filter-input')).forEach(inp => {
            ...
            if (isRegExp) {
                ...
                try {
                    ...
                    result.push({ val, idx: parseInt(inp.dataset.colIdx, 10) });
                } catch (e) {
                    // Invalid: 4 px error border + message in sub-table status span
                    inp.style.boxShadow   = '';
                    ...
                    if (statusSpan) {
                        ...
                        const colName = headers[colIdx]
                            ...
                            : `Col ${colIdx}`;
                        statusSpan.textContent = `⚠ Column "${colName}": invalid regexp: ${e.message}`;
