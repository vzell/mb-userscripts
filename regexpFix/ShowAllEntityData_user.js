    ...
    async function runFilter() {
        ...
        if (globalQueryRaw && isRegExp) {
            try {
                ...
                if (filterStatusDisplay) {
                    filterStatusDisplay.textContent = '⚠️ Invalid regex';
                    filterStatusDisplay.style.color = 'red';
                }
                await Lib.showCustomAlert(
                    `Invalid Regular Expression in global filter: ${e.message}\n\nPlease correct the pattern.`,
                    '❌ Invalid Regex',
                    filterInput
                );
        ...
        async function applySubFilter() {
            ...
            if (useRx) {
                try {
                    ...
                    filterInput.style.boxShadow = '0 0 0 3px rgba(255,0,0,0.35)';
                    await Lib.showCustomAlert(
                        `Invalid Regular Expression in sub-table filter: ${e.message}\n\nPlease correct the pattern.`,
                        '❌ Invalid Regex',
                        filterInput
                    );
                    return; // Abort — rows remain in current visible state, border stays red
                }
            } else {
