...
(function() {
    ...
    /**
     * Create a comparison function for multi-column table sorting
     * @param {Array} sortCriteria - Array of { index, direction, isNumeric }
     * @returns {Function} Comparison function
     */
    function createMultiSortComparator(sortCriteria) {
        return (a, b) => {
            for (const { index, direction, isNumeric } of sortCriteria) {
    ...
    // Sort Order Badges
    const multiSortStyle = document.createElement('style');
    multiSortStyle.id = 'mb-multi-sort-style';
    multiSortStyle.textContent = `
            .sort-icon-btn {
                position: relative;
                display: inline-flex;
                align-items: center;
            ...
            .sort-icon-active .sort-order-badge {
                background: #4caf50; /* Green badge for active sorts */
            }
        `;
    document.head.appendChild(multiSortStyle);
    ...
    function makeTableSortableUnified(table, sortKey) {
        ...
        if (!multiTableSortStates.has(sortKey)) {
            multiTableSortStates.set(sortKey, {
                sortCriteria: [] // Array of { index, direction, isNumeric, colName }
            });
        }
        const state = multiTableSortStates.get(sortKey);

        /**
         * Refreshes the visual state of all sort icons in the table
         */
        const updateSortUI = () => {
            ...
            state.sortCriteria.forEach((criteria, order) => {
                const th = headers[criteria.index];
                if (!th) return;

                // Target the specific ▲ or ▼ span
                const targetChar = criteria.direction === 1 ? '▲' : '▼';
                th.querySelectorAll('.sort-icon-btn').forEach(span => {
                    if (span.textContent === targetChar) {
                        span.classList.add('sort-icon-active');
                        // Add priority badge if there's more than one column being sorted
                        if (state.sortCriteria.length > 1) {
        ...
        headers.forEach((th, index) => {
            ...
            const isNumeric = colName.includes('#') || colName.toLowerCase().includes('date') ||
                             colName.toLowerCase().includes('time') || colName.toLowerCase().includes('count');
            ...
            const createIcon = (char, targetState) => {
                ...
                span.title = targetState === 0 ? 'Reset Sort' : (targetState === 1 ? 'Sort Ascending' : 'Sort Descending');
                ...
                span.onclick = async (e) => {
                    ...
                    // 1. Update Sort Criteria Stack
                    if (!isShift) {
                        // Regular click: reset to single column sort
                        state.sortCriteria = (targetState === 0) ? [] : [{ index, direction: targetState, isNumeric, colName }];
                    } else {
                        // Shift click: modify stack
                        if (targetState === 0) {
                            // Remove column from stack
                            state.sortCriteria = state.sortCriteria.filter(c => c.index !== index);
                        } else {
                            const existingIdx = state.sortCriteria.findIndex(c => c.index === index);
                            if (existingIdx > -1) {
                                // Update existing entry
                                state.sortCriteria[existingIdx].direction = targetState;
                            } else {
                                // Add new entry to end
                                state.sortCriteria.push({ index, direction: targetState, isNumeric, colName });
                            }
                        }
                    }

                    // 2. Prepare Data
                    let targetRows = isMultiTable ? groupedRows[parseInt(sortKey.split('_').pop(), 10)].rows : allRows;
                    let originalRows = isMultiTable ? groupedRows[parseInt(sortKey.split('_').pop(), 10)].originalRows : originalAllRows;

                    // 3. UI Feedback
                    updateSortUI();
                    const sortStatusDisplay = document.getElementById('mb-sort-status-display');
                    if (sortStatusDisplay) {
                        const colNames = state.sortCriteria.map(c => c.colName).join(' ➔ ');
                        sortStatusDisplay.textContent = state.sortCriteria.length ? `⏳ Sorting by: ${colNames}...` : '⏳ Resetting order...';
                        sortStatusDisplay.style.color = 'orange';
                    }

                    // 4. Perform Sort
                    document.body.classList.add('mb-sorting-active');
                    setTimeout(async () => {
                        try {
                            const startSort = performance.now();
                            let sortedData;

                            if (state.sortCriteria.length === 0) {
                                sortedData = [...originalRows];
                            } else {
                                sortedData = await sortLargeArray(
                                    [...targetRows],
                                    createMultiSortComparator(state.sortCriteria),
                                    progress => { /* update progress bar if needed */ }
                                );
                            }
                            ...
                            if (isMultiTable) {
                                ...
                                groupedRows[groupIndex].rows = sortedData;
                            ...
                            // 6. Cleanup
                            const duration = (performance.now() - startSort).toFixed(0);
                            if (sortStatusDisplay) {
                                sortStatusDisplay.textContent = `✓ Sorted in ${duration}ms`;
                                sortStatusDisplay.style.color = 'green';
                            }
                        } catch (err) {
                            Lib.error('sort', 'Multi-sort failed:', err);
                        } finally {
                            document.body.classList.remove('mb-sorting-active');
                        }
                    }, 10);
                };
                return span;
            ...
            iconGroup.appendChild(createIcon('⇅', 0));
            iconGroup.appendChild(createIcon('▲', 1));
            iconGroup.appendChild(createIcon('▼', 2));
            th.appendChild(iconGroup);
        });
