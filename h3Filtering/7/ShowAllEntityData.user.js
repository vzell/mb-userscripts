...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    async function startFetchingProcess(e, buttonConfig, baseDef) {
        ...
        try {
            ...
            makeH2sCollapsible();
            // Wire h3 collapse/expand click handlers (makeH3sCollapsible is also
            // called inside renderGroupedTable, but calling it here as well is
            // harmless — the data-mb-h3-wired guard prevents duplicate listeners)
            makeH3sCollapsible();
    ...
    async function renderGroupedTable(dataArray, isArtistMain, query = '') {
        ...
        dataArray.forEach((group, index) => {
            ...
            if (!query) {
                ...
                // h3 click handler is wired by makeH3sCollapsible() at the end of this
                // function — do NOT add an inline addEventListener here to avoid
                // double-wiring on subsequent renders.
        ...
        // Wire/re-wire click handlers on all current .mb-toggle-h3 elements.
        // Called here (not only at the top-level render site) so that the disk-load
        // path also benefits from a fresh wiring pass after the new h3s are in the DOM.
        makeH3sCollapsible();
    ...
    /**
     * Wires (or re-wires) click handlers on every current `.mb-toggle-h3` element.
     *
     * Design:
     *  - Uses a `data-mb-h3-wired` attribute to avoid attaching duplicate listeners
     *    on elements that are already wired (e.g. during runFilter re-renders that
     *    reuse existing h3 elements).
     *  - Derives the associated `<table>` element at click-time via
     *    `h3.nextElementSibling` so no stale closure reference is held.
     *  - Must be called after every fresh render that creates new `.mb-toggle-h3`
     *    elements — specifically at the end of `renderGroupedTable()` and after the
     *    disk-load re-render inside `reader.onload`.
     *  - The guard clause intentionally allows clicks on `.mb-filter-status` and
     *    `.mb-sort-status` spans inside `.mb-subtable-controls` to pass through;
     *    those status labels are meant to trigger the section toggle just like
     *    clicking the h3 text itself.  Buttons and interactive form elements inside
     *    `.mb-subtable-controls` are still blocked.
     *
     * @param {string} [logContext='render'] - Debug log channel label.
     */
    function makeH3sCollapsible(logContext = 'render') {
        const h3s = document.querySelectorAll('.mb-toggle-h3');
        Lib.debug(logContext, `makeH3sCollapsible: wiring ${h3s.length} h3 header(s)`);

        h3s.forEach(h3 => {
            if (h3.dataset.mbH3Wired) return; // already wired — skip to avoid duplicates
            h3.dataset.mbH3Wired = '1';

            h3.addEventListener('click', (e) => {
                // ── Guard: block clicks on interactive elements -----------------
                // Allow .mb-filter-status and .mb-sort-status inside controls —
                // those status labels should trigger the section toggle.
                const inControls = e.target.closest('.mb-subtable-controls');
                if (
                    ['A', 'BUTTON', 'INPUT', 'LABEL', 'SELECT', 'TEXTAREA'].includes(e.target.tagName) ||
                    (inControls &&
                     !e.target.classList.contains('mb-filter-status') &&
                     !e.target.classList.contains('mb-sort-status')) ||
                ...
                // Resolve the associated table at click-time (never a stale reference)
                const tbl = h3.nextElementSibling;
                if (!tbl || tbl.tagName !== 'TABLE') return;

                if (e.ctrlKey || e.metaKey) {
                    // ── Ctrl+Click: toggle ALL h3 sections ─────────────────────
                    const isExpanding = tbl.style.display === 'none';
                    document.querySelectorAll('.mb-toggle-h3').forEach(otherH3 => {
                        const otherTbl = otherH3.nextElementSibling;
                        if (otherTbl && otherTbl.tagName === 'TABLE') {
                            otherTbl.style.display = isExpanding ? '' : 'none';
                        }
                        const otherIcon = otherH3.querySelector('.mb-toggle-icon');
                        if (otherIcon) otherIcon.textContent = isExpanding ? '▼' : '▲';
                    });
                    Lib.debug(logContext, `Ctrl+Click: all h3 sections ${isExpanding ? 'shown' : 'hidden'}`);
                } else {
                    // ── Normal click: toggle just this section ──────────────────
                    const isHidden = tbl.style.display === 'none';
                    tbl.style.display = isHidden ? '' : 'none';
                    const toggleIcon = h3.querySelector('.mb-toggle-icon');
                    if (toggleIcon) toggleIcon.textContent = isHidden ? '▼' : '▲';
                    Lib.debug(logContext, `h3 "${h3.textContent.trim().substring(0,30)}": toggled to ${isHidden ? 'visible' : 'hidden'}`);
                }
            });
        });
    }
     ...
     *  - `h2._mbToggle` traverses the DOM *live* (at call-time) instead of using a
     *    snapshot captured at init-time.  This means a second "load from disk" render
     *    that replaces the old h3/table siblings will always be controlled correctly
     *    by the existing h2 toggle — no need to re-call makeH2sCollapsible() after a
     *    fresh render.
     *  - The `#mb-filter-status-display` span (global filter status shown inside the
     *    h2) is wired up to also trigger the h2 toggle, so the user can click the
     *    status text as well as the h2 background area.
     */
    function makeH2sCollapsible() {
        ...
        allH2s.forEach(h2 => {
            ...
            // ------------------------------------------------------------------
            // Helper: collect the current live siblings between this H2 and the
            // next H2.  Called at toggle-time so a replaced subtable set (e.g.
            // after a disk-load re-render) is always seen correctly.
            // ------------------------------------------------------------------
            const getLiveContentNodes = () => {
                const nodes = [];
            ...
            // Hide content if not the main data header (use snapshot once at init)
            if (!isMainDataHeader) {
                getLiveContentNodes().forEach(node => { node.style.display = 'none'; });
            }
            ...
            // 1. Define Toggle Function on the Element (uses live DOM traversal)
            h2._mbToggle = (forceExpand) => {
                ...
                getLiveContentNodes().forEach(node => {
                    if (shouldExpand) {
            ...
            // 3. Also allow the global filter-status display to act as a toggle trigger.
            //    The span lives inside the filterContainer (which is appended to this h2)
            //    and is normally excluded by the filterContainer guard above.  We wire it
            //    directly so the user can click the status text to collapse/expand.
            if (isMainDataHeader) {
                const wireFilterStatusClick = () => {
                    const statusSpan = document.getElementById('mb-filter-status-display');
                    if (statusSpan && !statusSpan.dataset.mbH2ToggleWired) {
                        statusSpan.dataset.mbH2ToggleWired = '1';
                        statusSpan.style.cursor = 'pointer';
                        statusSpan.title = 'Click to collapse/expand the section';
                ...
                // Wire immediately if already in DOM, otherwise defer briefly
                wireFilterStatusClick();
                setTimeout(wireFilterStatusClick, 500);
            }
        });
    }
    ...
    async function loadTableDataFromDisk(file, filterQueryRaw = '', isCaseSensitive = false, isRegExp = false, isExclude = false) {
        ...
        reader.onload = async (e) => {
            try {
                ...
                // ── Reset global filter state so it does not bleed into the freshly
                //    loaded data.  Any active filter string stays visible in the input
                //    only when it was intentionally submitted as a pre-filter; in all
                //    other cases the field must be cleared to avoid a visual lie where
                //    the input shows a filter string that is not actually applied.
                if (filterInput.value && !filterQueryRaw) {
                    // There was a live global filter but no pre-filter was used for this
                    // disk load → clear the field so the page starts clean.
                    // (Don't call runFilter here; the full re-render below takes care of the DOM.)
                    filterInput.value = '';
                    Lib.debug('cache', 'Cleared stale global filter input before disk-load render');
                }
                ...
                finalCleanup();
                makeH2sCollapsible();
                // Re-wire h3 click handlers for the newly rendered h3 elements.
                // makeH3sCollapsible() is already called inside renderGroupedTable
                // via the closing hook, but an explicit call here ensures the wiring
                // is correct even if the render path changed.  The data-mb-h3-wired
                // guard makes the extra call a safe no-op for already-wired h3s.
                makeH3sCollapsible('cache');
                // Wire the global filter-status display click → h2 toggle (may have
                // been added to the DOM only during this disk-load render).
                setTimeout(() => {
                    const statusSpan = document.getElementById('mb-filter-status-display');
                    if (statusSpan && !statusSpan.dataset.mbH2ToggleWired) {
                        const targetH2 = document.querySelector('h2.mb-toggle-h2');
                        if (targetH2 && typeof targetH2._mbToggle === 'function') {
                            const icon = targetH2.querySelector('.mb-toggle-icon');
                            statusSpan.dataset.mbH2ToggleWired = '1';
                            statusSpan.style.cursor = 'pointer';
                            statusSpan.title = 'Click to collapse/expand the section';
                            statusSpan.addEventListener('click', (e) => {
                                ...
                                targetH2._mbToggle(isExpanding);
                            });
                        }
                    }
                }, 100);
