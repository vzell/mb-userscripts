...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function createSubTableFilterContainer(categoryName, table) {
        ...
        /**
         * Updates the (N of M) row-count badge in the h3 that precedes this table.
         *
         * Denominator = rows not hidden by the global/column filter
         *               (i.e. visible rows + rows hidden only by the subtable filter).
         * Numerator   = rows currently visible (not hidden by anything).
         *
         * When the subtable filter is empty the badge reverts to plain "(M)".
         */
        function updateSubTableRowCount() {
            ...
            // Denominator: rows that were visible before the subtable filter ran
            ...
            // Numerator: rows that are visible right now
            const visible = allTbodyRows.filter(r => r.style.display !== 'none').length;

            if (filterInput.value) {
                countStat.textContent = `(${visible} of ${denominator})`;
            } else {
                // Filter cleared – show just the denominator (= rows after global filter)
                countStat.textContent = `(${denominator})`;
            }
        }
