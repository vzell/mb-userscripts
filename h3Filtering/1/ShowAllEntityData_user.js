...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    function createSubTableFilterContainer(categoryName, table) {
        ...
        function applySubFilter() {
            ...
            // Clear existing subtable highlights from ALL rows (visible + hidden)
            if (table) {
            ...
            if (useRx) {
                try {
                    ...
                    return; // Invalid regexp – stop here
                }
            } else {
            ...
            // Iterate only over rows that are currently visible (not hidden by global/column filter)
            Array.from(table.querySelectorAll('tbody tr')).forEach(row => {
                if (row.style.display === 'none') return; // already hidden by global/column filter
                    ...
                    if (highlightEnabled && !useEx) {
                        row.querySelectorAll('td').forEach(td => {
                            ...
                            hits.forEach(textNode => {
                                ...
                                wrap.innerHTML = textNode.nodeValue.replace(
                                    regex,
