...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    clearAllFiltersBtn.onclick = () => {
        ...
        // Call the main clearAllFilters function (clears global + all column filters)
        clearAllFilters();

        // Also explicitly clear per-subtable filter status displays so each sub-table shows cleared state
     ...
     * Supports a three-tier format when both global and column/sub-table filters are active:
     *   - No filter active          : (total)
     *   - Only global filter        : (globalFiltered of total)
     *   - Only column filter        : (colFiltered of total)
     *   - Global + column filter    : (colFiltered of globalFiltered)/total
     * @param {number} filteredCount       - Number of rows currently visible after ALL filters
     * @param {number} totalCount          - Total number of rows before any filtering
     * @param {number} [globalFilteredCount] - Rows after global filter only (before column filters);
     *                                        defaults to filteredCount when not supplied (no column filter)
     */
    function updateH2Count(filteredCount, totalCount, globalFilteredCount = filteredCount) {
        Lib.debug('render', `Starting updateH2Count: filtered=${filteredCount}, globalFiltered=${globalFilteredCount}, total=${totalCount}`);
        ...
        if (targetH2) {
            ...
            // Build count text using three-tier logic:
            //   filteredCount == totalCount          → no filter          : (total)
            //   filteredCount == globalFilteredCount → only global filter  : (globalFiltered of total)
            //      (and globalFilteredCount < totalCount, otherwise truly no filter)
            //   filteredCount < globalFilteredCount  → column filter also  :
            //      globalFilteredCount < totalCount  → both filters        : (col of global)/total
            //      globalFilteredCount == totalCount → column filter only  : (col of total)
            let countText;
            if (filteredCount === totalCount) {
                countText = `(${totalCount.toLocaleString()})`;
            } else if (filteredCount === globalFilteredCount) {
                // Only global filter active (column filter did not reduce further)
                countText = `(${filteredCount.toLocaleString()} of ${totalCount.toLocaleString()})`;
            } else if (globalFilteredCount < totalCount) {
                // Both global and column filters active
                countText = `(${filteredCount.toLocaleString()} of ${globalFilteredCount.toLocaleString()})/${totalCount.toLocaleString()}`;
            } else {
                // Only column filter active, no global filter
                countText = `(${filteredCount.toLocaleString()} of ${totalCount.toLocaleString()})`;
            }
    ...
    function runFilter() {
        ...
        if (activeDefinition.tableMode === 'multi') {
            ...
            // Context for global-filter-only pass (no column filters) — used to compute
            // the intermediate "globally filtered" count per group for the three-tier display.
            const globalOnlyCtx = { globalQuery, globalQueryRaw, globalRegex,
                                    isCaseSensitive, isRegExp, isExclude, colFilters: [] };
            ...
            groupedRows.forEach((group, groupIdx) => {
                ...
                // Compute globally-filtered-only count (ignores column filters)
                const globalFilteredRows = globalQueryRaw
                    ? group.rows.filter(r => testRowMatch(r.cloneNode(true), globalOnlyCtx))
                    : group.rows;
                const groupGlobalFilteredCount = globalFilteredRows.length;
                totalGlobalFiltered += groupGlobalFilteredCount;
                ...
                filteredArray.push({
                    category: group.category || group.key || 'Unknown',
                    rows: matches,
                    globalFilteredCount: groupGlobalFilteredCount,
            ...
            updateH2Count(totalFiltered, totalAbsolute, totalGlobalFiltered);
        } else {
            ...
            // Compute globally-filtered-only count for the three-tier h2 display
            const globalOnlyCtx = { globalQuery, globalQueryRaw, globalRegex,
                                    isCaseSensitive, isRegExp, isExclude, colFilters: [] };
            const globalFilteredCount = globalQueryRaw
                ? allRows.filter(r => testRowMatch(r.cloneNode(true), globalOnlyCtx)).length
                : totalAbsolute;
            ...
            updateH2Count(filteredRows.length, totalAbsolute, globalFilteredCount);
        }
    ...
    async function renderGroupedTable(dataArray, isArtistMain, query = '') {
        ...
        dataArray.forEach((group, index) => {
            ...
            if (!query) {
                ...
                // Update the count in the header during filtering using three-tier display logic:
                //   no filter            : (total)
                //   global only          : (globalFiltered of total)
                //   column only          : (colFiltered of total)
                //   global + column      : (colFiltered of globalFiltered)/total
                const countStat = h3.querySelector('.mb-row-count-stat');
                const totalCount = group.totalCount
                    || groupedRows.find(g => (g.category || g.key || 'Unknown') === categoryName)?.rows.length
                    || 0;
                const globalFilteredCount = (group.globalFilteredCount !== undefined)
                    ? group.globalFilteredCount
                    : totalCount;
                const localFilteredCount = group.rows.length;

                if (countStat) {
                    let h3CountText;
                    if (localFilteredCount === totalCount) {
                        h3CountText = `(${totalCount.toLocaleString()})`;
                    } else if (localFilteredCount === globalFilteredCount) {
                        // Only global filter active (column filter did not reduce further)
                        h3CountText = `(${localFilteredCount.toLocaleString()} of ${totalCount.toLocaleString()})`;
                    } else if (globalFilteredCount < totalCount) {
                        // Both global and column filters active
                        h3CountText = `(${localFilteredCount.toLocaleString()} of ${globalFilteredCount.toLocaleString()})/${totalCount.toLocaleString()}`;
                    } else {
                        // Only column filter active, no global filter
                        h3CountText = `(${localFilteredCount.toLocaleString()} of ${totalCount.toLocaleString()})`;
                    }
                    countStat.textContent = h3CountText;
                }
