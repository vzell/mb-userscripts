...
(function() {
    ...
    /**
     * Build a per-subtable filter container (input + Cc/Rx/Ex checkboxes) modelled on
     * the global filter bar.  IDs are made unique by incorporating a sanitised version
     * of the category name so multiple instances can coexist on the same page.
     *
     * The container is initially hidden; the caller is responsible for toggling the
     * `visible` CSS class (or `.style.display`) as needed.
     *
     * @param {string} categoryName - Human-readable subtable category name used to
     *                                derive unique element IDs and the placeholder text.
     * @param {HTMLElement} table   - The <table> element this filter will operate on.
     * @returns {{ container: HTMLElement, input: HTMLInputElement,
     *             caseCheckbox: HTMLInputElement, regexpCheckbox: HTMLInputElement,
     *             excludeCheckbox: HTMLInputElement }} - The container and its inputs.
     */
    function createSubtableFilterContainer(categoryName, table) {
        // Derive a DOM-safe ID prefix from the category name
        const idSlug = categoryName.toLowerCase().replace(/[^a-z0-9]/g, '-').replace(/-+/g, '-').replace(/^-|-$/g, '') || 'subtable';
        const idPfx  = `mb-stf-${idSlug}`;

        const container = document.createElement('span');
        container.className = 'mb-subtable-filter-container';
        container.dataset.categoryName = categoryName;

        // --- Input wrapper (relative positioning for the ✕ clear icon) ---
        const wrapper = document.createElement('span');
        wrapper.className = 'mb-subtable-filter-wrapper';

        const input = document.createElement('input');
        input.id          = `${idPfx}-input`;
        input.placeholder = `🔍 Filter "${categoryName}"…`;
        input.title       = `Filter rows in the "${categoryName}" sub-table`;
        input.style.cssText = uiGlobalFilterInputCSS();
        // Slightly narrower than the global filter so it fits in the h3 line comfortably
        input.style.width = '320px';

        const clearIcon = document.createElement('span');
        clearIcon.id       = `${idPfx}-clear`;
        clearIcon.textContent = '✕';
        clearIcon.title    = `Clear filter for "${categoryName}"`;
        clearIcon.style.cssText = 'position:absolute; right:5px; top:50%; transform:translateY(-50%); cursor:pointer; font-size:0.6em; color:#999; user-select:none;';
        ...
        caseCheckbox.id   = `${idPfx}-case-checkbox`;
        ...
        rxLabel.id = `${idPfx}-rx-label`;
        rxLabel.title = 'RegExp Filtering';
        rxLabel.style.cssText = `${uiCheckboxLabelCSS()} font-weight:normal; height:24px;`;
        const rxCheckbox = document.createElement('input');
        rxCheckbox.id   = `${idPfx}-rx-checkbox`;
        rxCheckbox.type = 'checkbox';
        rxCheckbox.style.cssText = uiCheckboxInputCSS();
        rxLabel.appendChild(rxCheckbox);
        rxLabel.appendChild(document.createTextNode('Rx'));

        // --- Ex checkbox ---
        const exLabel = document.createElement('label');
        exLabel.id = `${idPfx}-ex-label`;
        exLabel.title = `Exclude matches (Filter for "${categoryName}")`;
        exLabel.style.cssText = `${uiCheckboxLabelCSS()} font-weight:normal; height:24px;`;
        const exCheckbox = document.createElement('input');
        exCheckbox.id   = `${idPfx}-ex-checkbox`;
        exCheckbox.type = 'checkbox';
        exCheckbox.style.cssText = uiCheckboxInputCSS();
        ...
        // --- Wire up the live-filter behaviour ---
        const applySubFilter = () => {
            const rawVal      = input.value;
            const isCaseSens  = caseCheckbox.checked;
            const isRx        = rxCheckbox.checked;
            const isExcl      = exCheckbox.checked;
            const queryNorm   = (isCaseSens || isRx) ? rawVal : rawVal.toLowerCase();

            // Style the input border to signal regexp validity
            if (rawVal && isRx) {
                try { new RegExp(rawVal); input.style.border = '2px solid #ccc'; }
                catch (_) { input.style.border = '2px solid red'; }
            } else {
                input.style.border = '2px solid #ccc';
            }
            input.style.boxShadow = rawVal ? '0 0 2px 2px red' : '';

            // Iterate over all data rows in the table (skip filter row, header)
            const rows = Array.from(table.querySelectorAll('tbody tr:not(.mb-col-filter-row)'));
            let visibleCount = 0;
            rows.forEach(row => {
                const text = (isCaseSens || isRx) ? row.textContent : row.textContent.toLowerCase();
                let match = false;
                if (isRx) {
                    try { match = new RegExp(queryNorm, isCaseSens ? '' : 'i').test(text); }
                    catch (_) { match = text.includes(queryNorm); }
                } else {
                    match = text.includes(queryNorm);
                }
                const show = rawVal ? (isExcl ? !match : match) : true;
                row.style.display = show ? '' : 'none';
                if (show) visibleCount++;
            });

            // Update the subtable row-count stat
            const h3 = table.previousElementSibling;
            if (h3) {
                const countStat = h3.querySelector('.mb-row-count-stat');
                if (countStat) {
                    const totalRows = rows.length;
                    countStat.textContent = (visibleCount === totalRows || !rawVal)
                        ? `(${totalRows})`
                        : `(${visibleCount} of ${totalRows})`;
                }
            }
        };

        input.addEventListener('input', applySubFilter);
        caseCheckbox.addEventListener('change', applySubFilter);
        rxCheckbox.addEventListener('change', applySubFilter);
        exCheckbox.addEventListener('change', applySubFilter);

        // Prevent h3 click-collapse from firing when interacting with the filter container
        container.addEventListener('click', e => e.stopPropagation());

        return { container, input, caseCheckbox, regexpCheckbox: rxCheckbox, excludeCheckbox: exCheckbox };
    }
...
<div style="color:#555; margin-bottom:18px; font-size:0.9em;">${error}</div>`;
    ...
    style.textContent = `
        ...
        .mb-subtable-filter-toggle {
            cursor: pointer; font-size: 1.0em; vertical-align: middle; user-select: none;
            opacity: 0.7; margin-left: 6px; transition: opacity 0.15s;
        }
        .mb-subtable-filter-toggle:hover { opacity: 1; }
        .mb-subtable-filter-toggle.active { opacity: 1; color: #c00; }
        .mb-subtable-filter-container {
            display: none; align-items: center; white-space: nowrap; gap: 5px;
            margin-left: 8px; vertical-align: middle;
        }
        .mb-subtable-filter-container.visible { display: inline-flex; }
        .mb-subtable-filter-wrapper { position: relative; display: inline-block; }
    ...
    async function renderGroupedTable(dataArray, isArtistMain, query = '') {
        ...
        dataArray.forEach((group, index) => {
            ...
            if (query && existingTables[index]) {
                ...
                // Ensure 🔍 subtable-filter toggle icon exists when reusing h3
                if (h3 && !h3.querySelector('.mb-subtable-filter-toggle')) {
                    const { container: stfContainer } = createSubtableFilterContainer(categoryName, table);
                    const filterToggleIcon = document.createElement('span');
                    filterToggleIcon.className = 'mb-subtable-filter-toggle';
                    filterToggleIcon.textContent = '🔍';
                    filterToggleIcon.title = 'Toggle filter elements on/off';
                    filterToggleIcon.addEventListener('click', (e) => {
                        e.stopPropagation();
                        const isActive = filterToggleIcon.classList.toggle('active');
                        stfContainer.classList.toggle('visible', isActive);
                        if (isActive) {
                            const inp = stfContainer.querySelector('input[type="text"], input:not([type])');
                            if (inp) setTimeout(() => inp.focus(), 50);
                        }
                    });
                    const rowCountStat = h3.querySelector('.mb-row-count-stat');
                    if (rowCountStat) {
                        rowCountStat.insertAdjacentElement('afterend', filterToggleIcon);
                        filterToggleIcon.insertAdjacentElement('afterend', stfContainer);
                    } else {
                        h3.appendChild(filterToggleIcon);
                        h3.appendChild(stfContainer);
                    }
                }
            } else {
            ...
            if (!query) {
                ...
                // Add 🔍 subtable-filter toggle icon after the row-count stat
                const { container: stfContainer } = createSubtableFilterContainer(categoryName, table);
                const filterToggleIcon = document.createElement('span');
                filterToggleIcon.className = 'mb-subtable-filter-toggle';
                filterToggleIcon.textContent = '🔍';
                ...
                // Insert toggle icon immediately after .mb-row-count-stat inside h3
                const rowCountStat = h3.querySelector('.mb-row-count-stat');
                rowCountStat.insertAdjacentElement('afterend', filterToggleIcon);
                filterToggleIcon.insertAdjacentElement('afterend', stfContainer);
                ...
                // Ensure 🔍 subtable-filter toggle icon exists (may be absent if h3 was created during initial filter)
                if (!h3.querySelector('.mb-subtable-filter-toggle')) {
                    const { container: stfContainer } = createSubtableFilterContainer(categoryName, table);
                    const filterToggleIcon = document.createElement('span');
                    filterToggleIcon.className = 'mb-subtable-filter-toggle';
                    filterToggleIcon.textContent = '🔍';
