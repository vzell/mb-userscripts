// ==UserScript==
// @name         MusicBrainz Customizable PrimaryType Selector
// @namespace    https://github.com/vzell/mb-userscripts
// @version      1.0
// @description  Add customizable quick- button for primaryType in MusicBrainz release editor
// @author       YoGo9 & vzell
// @homepage     https://github.com/vzell/mb-userscripts
// @updateURL    https://raw.githubusercontent.com/vzell/mb-userscripts/master/CustomizablePrimaryTypeSelector.user.js
// @downloadURL  https://raw.githubusercontent.com/vzell/mb-userscripts/master/CustomizablePrimaryTypeSelector.user.js
// @supportURL   https://github.com/vzell/Scripts/issues
...
(function() {
    ...
    // Default settings - modify these for your preferred primaryTypees
    const DEFAULT_PRIMARYTYPE = ['Album', 'EP', 'Single'];

    // Load user settings or use defaults
    let preferredPrimaryType = GM_getValue('mbPrimaryType', DEFAULT_PRIMARYTYPE);
    ...
    function addButtonsAfter(element, buttons, isMultiSelect = false) {
        ...
        settingsBtn.addEventListener('click', function(e) {
            ...
            showSettingsDialog(type, getAvailableOptions(element), preferredPrimaryType);
            return false;
        });
    ...
    function showSettingsDialog(type, availableOptions, selectedOptions) {
        ...
        header.textContent = 'Customize PrimaryType Button';
        ...
        saveButton.addEventListener('click', function() {
            ...
            preferredPrimaryType = newSelection;
            GM_setValue('mbPrimaryType', newSelection);
	...
	// Function for updating the release editor primaryType UI
    function updateReleaseEditorPrimaryTypeUI() {
        // Make sure the "Add primaryType" button is visible
        const addPrimaryTypeButton = document.getElementById('add-primary-type');
        if (addPrimaryTypeButton) {
            addPrimaryTypeButton.style.display = '';
        }

        // Show all primaryType rows
        const primaryTypeRows = document.querySelectorAll('.select-list-row');
        for (let i = 0; i < primaryTypeRows.length; i++) {
            primaryTypeRows[i].style.display = '';
        }
    }

    // Function to add release editor primaryType button
	function addReleaseEditorPrimaryTypeButton() {
        // Check if the primaryType rows container exists
        const primaryTypeRowsContainer = document.querySelector('.form-row-select-list');
        if (!primaryTypeRowsContainer) return;
        ...
        // Get the first primaryType select for option lookup
        const firstPrimaryTypeSelect = document.querySelector('.select-list-row select');
        if (!firstPrimaryTypeSelect) return;

        // Add buttons for each preferred primaryType
        preferredPrimaryType.forEach(primaryType => {
            const btn = document.createElement('button');
            btn.textContent = primaryType;
            btn.type = 'button';
            ...
            // Get the primaryType value
            const value = findOptionValueByText(firstPrimaryTypeSelect, primaryType);
            if (!value) {
                // Skip this primaryType if it doesn't exist in the dropdown
                return;
            }
            ...
            btn.addEventListener('click', function(e) {
                e.preventDefault();
                // Regular primaryType handling - find or create an empty dropdown
                ...
                if (!emptySelect) {
                    // If no empty select found, click the "Add primaryType" button to add a new row
                    // and immediately set the value (to avoid the two-step click)
                    const addPrimaryTypeButton = document.getElementById('add-primary-type');
                    if (addPrimaryTypeButton) {
                        // Temporarily store the original click handler
                        const originalClick = addPrimaryTypeButton.onclick;

                        // Override the click handler to add our custom logic
                        addPrimaryTypeButton.onclick = function(e) {
                            ...
                            setTimeout(() => {
                                ...
                                addPrimaryTypeButton.onclick = originalClick;
                            }, 50);
                        };

                        // Trigger the click to add a new row
                        addPrimaryTypeButton.click();
                    }
                } else {
	...
	// Add settings button
        ...
        settingsBtn.addEventListener('click', function(e) {
            e.preventDefault();
            showSettingsDialog('primaryType', getAvailableOptions(firstPrimaryTypeSelect), preferredPrimaryType);
            return false;
        });
        ...
        let primaryTypeLabel = null;
        const labels = document.querySelectorAll('label');
        for (let i = 0; i < labels.length; i++) {
            if (labels[i].textContent.includes('PrimaryTypes')) {
                primaryTypeLabel = labels[i];
                break;
            }
        }
        ...
        if (primaryTypeLabel) {
            ...
            const parentElement = primaryTypeLabel.parentElement;
            if (parentElement && parentElement.parentElement) {
                parentElement.parentElement.insertBefore(quickAddContainer, primaryTypeRowsContainer.nextSibling);
            } else {
                // Fallback insertion
                primaryTypeRowsContainer.parentElement.insertBefore(quickAddContainer, primaryTypeRowsContainer.nextSibling);
            }
        } else {
            // Fallback - insert after the primaryType rows container
            primaryTypeRowsContainer.parentElement.insertBefore(quickAddContainer, primaryTypeRowsContainer.nextSibling);
        }

        // Set up observers to watch for primaryType changes
        const firstPrimaryTypeSelectObserver = new MutationObserver(function(mutations) {
            updateReleaseEditorPrimaryTypeUI();
        });

        // Observe changes to the first primaryType select value
        firstPrimaryTypeSelectObserver.observe(firstPrimaryTypeSelect, {
            attributes: true,
    ...
    window.addEventListener('load', function() {
        // Add button for primaryType selection in release editor
        const primaryTypeSelect = document.getElementById('primary-type');
        if (primaryTypeSelect) {
            const primaryTypeButtons = createButtonsFromPreferences(primaryTypeSelect, preferredPrimaryType);
            addButtonsAfter(primaryTypeSelect, primaryTypeButtons);
        }
    });
})();
