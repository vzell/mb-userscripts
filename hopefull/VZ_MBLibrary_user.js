...
//     {version: '3.2.0+2026-02-23', description: 'Settings modal ghost-row fix (root cause): the border-bottom:1px solid rgba(255,255,255,0.2) was placed on the <tr class="section-header"> element. With border-collapse:collapse, browsers can render inter-row border space even when adjacent config-rows are display:none, causing each collapsed section header to contribute exactly 1px of white to a cumulative ghost strip at the bottom of the table. Fix: moved border-bottom from the <tr> to the <td colspan="3"> child. Borders on <td> are clipped to the cell box and cannot bleed into empty table space, so the ghost strip disappears completely when all sections are collapsed.'},
