...
// CHANGELOG
let changelog = [
    {version: '1.9.0+2026-02-03', description: 'Add Save to Disk and Load from Disk functionality for offline viewing.'},
...
(function() {
    ...
    saveBtn.style.cssText = 'font-size:0.5em; padding:2px 6px; cursor:pointer; height:24px; box-sizing:border-box; margin-left:4px; display:none;';
    saveBtn.title = 'Save the current consolidated view to a JSON file on your disk';

    const loadBtn = document.createElement('button');
    loadBtn.textContent = 'Load from Disk';
    loadBtn.style.cssText = 'font-size:0.5em; padding:2px 6px; cursor:pointer; height:24px; box-sizing:border-box; margin-left:4px;';
    loadBtn.title = 'Load a previously saved consolidated view file';
    ...
    saveBtn.onclick = () => {
        ...
        if (groupedRows && groupedRows.length > 0) {
            dataToSave = {
                type: 'grouped',
                data: groupedRows.map(g => ({
                    category: g.category,
                    rows: g.rows.map(r => r.innerHTML)
                }))
            };
        } else {
            dataToSave = {
                type: 'flat',
                data: allRows.map(r => r.innerHTML)
            };
        }
        ...
        const fileName = `MB_${pageType}_${new Date().getTime()}.json`;
        ...
        Lib.info('save', `Data saved to file: ${fileName}`);
    };

    // Logic: Load from Disk
    ...
    fileInput.onchange = (e) => {
        ...
        reader.onload = (event) => {
            try {
                ...
                if (json.pageType !== pageType) {
                    if (!confirm(`Warning: File is for ${json.pageType} but current page is ${pageType}. Continue?`)) return;
                }

                performClutterCleanup();
                stopOtherScripts();
                allActionButtons.forEach(b => b.style.display = 'none');
                loadBtn.style.display = 'none';

                if (json.content.type === 'grouped') {
                    groupedRows = json.content.data.map(g => {
                        return {
                            category: g.category,
                            rows: g.rows.map(html => {
                    ...
                    allRows = json.content.data.map(html => {
                        const tr = document.createElement('tr');
                        tr.innerHTML = html;
                        return tr;
                    });
                    originalAllRows = [...allRows];
                    renderFlatTable(allRows);
                }
                ...
                initSidebarCollapse();
                runFilter();
                Lib.info('load', 'Data re-hydrated from disk successfully.');
            } catch (err) {
                Lib.error('load', 'Failed to load file:', err);
    ...
    function runFilter() {
        ...
        if (pageType === 'releasegroup-releases' || pageType === 'artist-releasegroups') {
            ...
            if (__activeEl && __activeEl.classList.contains('mb-col-filter-input')) {
                const colIdx = __activeEl.dataset.colIdx;
                const newInput = document.querySelector(`.mb-col-filter-input[data-col-idx="${colIdx}"]`);
                if (newInput) {
                    newInput.focus();
                    newInput.selectionStart = newInput.selectionEnd = newInput.value.length;
                }
    ...
    async function startFetchingProcess(e, customParams = null) {
        if (isLoaded) return;
        loadBtn.style.display = 'none';

        const btn = e.target;
        btn.classList.add('mb-show-all-btn-active', 'mb-show-all-btn-loading');
        btn.disabled = true;
        ...
        stopBtn.onclick = () => {
            ...
            Lib.info('fetch', 'Stop requested by user.');
        };

        const targetPath = (pageType === 'artist-releasegroups' || (pageType === 'releases' && path.includes('/artist/'))) ? path : path;
        const fetchParams = customParams || Object.fromEntries(params.entries());
        const maxPage = await fetchMaxPageGeneric(targetPath, fetchParams);

        if (maxPage > Lib.settings.sa_max_page) {
            if (!confirm(`This will fetch ${maxPage} pages. Continue?`)) {
                btn.disabled = false;
                btn.classList.remove('mb-show-all-btn-active', 'mb-show-all-btn-loading');
        ...
        performClutterCleanup();
        stopOtherScripts();
        allActionButtons.forEach(b => { if (b !== btn) b.style.display = 'none'; });

        progressContainer.style.display = 'inline-block';
        timerDisplay.style.display = 'inline-flex';
        progressBar.style.width = '0%';
        progressBar.style.backgroundColor = '#4caf50';
        progressText.textContent = `0 / ${maxPage} pages (0%)`;
        ...
        for (let p = 1; p <= maxPage; p++) {
            ...
            const url = new URL(window.location.origin + targetPath);
            Object.keys(fetchParams).forEach(k => url.searchParams.set(k, fetchParams[k]));
            url.searchParams.set('page', p.toString());

            statusDisplay.textContent = `Fetching page ${p}...`;
            try {
                const html = await fetchHtml(url.toString());
                const doc = new DOMParser().parseFromString(html, 'text/html');
                ...
                if (table) {
                    if (pageType === 'artist-releasegroups' || pageType === 'releasegroup-releases') {
                        ...
                        children.forEach(child => {
                            if (child.tagName === 'TR' && (child.classList.contains('subh') || child.querySelector('th[colspan]'))) {
                                currentCategory = child.textContent.trim();
                            } else if (child.tagName === 'TR') {
                                if (!rowsByGroup.has(currentCategory)) rowsByGroup.set(currentCategory, []);
                                rowsByGroup.get(currentCategory).push(child);
                            }
                        });
                    } else {
                        const pageRows = Array.from(table.querySelectorAll('tbody tr:not(.subh)'));
                ...
                const avg = elapsed / p;
                const remaining = avg * (maxPage - p);
                const percent = Math.round((p / maxPage) * 100);

                progressBar.style.width = `${percent}%`;
                progressBar.style.backgroundColor = p === maxPage ? '#2196F3' : '#4caf50';
                progressText.textContent = `${p} / ${maxPage} pages (${percent}%) - Est. ${Math.round(remaining)}s remaining`;
                timerDisplay.textContent = `Elapsed: ${Math.round(elapsed)}s`;

            } catch (err) {
                Lib.error('fetch', `Error fetching page ${p}:`, err);
            }
        }

        if (pageType === 'artist-releasegroups' || pageType === 'releasegroup-releases') {
            groupedRows = Array.from(rowsByGroup.entries()).map(([cat, rows]) => ({ category: cat, rows }));
            renderGroupedTable(groupedRows, pageType === 'artist-releasegroups');
        } else {
            originalAllRows = [...allRows];
            renderFlatTable(allRows);
        ...
        saveBtn.style.display = 'inline-block';
        filterContainer.style.display = 'inline-flex';
        updateH2Count(allRows.length || groupedRows.reduce((a, b) => a + b.rows.length, 0), allRows.length || groupedRows.reduce((a, b) => a + b.rows.length, 0));

        initSidebarCollapse();
    }

    // Remaining rendering functions (renderFlatTable, renderGroupedTable, fetchHtml, etc.) would follow here as in your source.
    // Ensure all logic for table creation and filtering is kept identical to NewConsolidated.js.
