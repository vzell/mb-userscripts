...
let changelog = [
    {version: '0.9.3+2026-01-29', description: 'Added visual progress bar; Improved rendering time calculation; Busy cursor during initial render.'},
...
(function() {
    ...
    progressBarContainer.style.cssText = 'display:none; width:150px; height:12px; border:1px solid #ccc; background:#eee; position:relative; overflow:hidden; border-radius:2px; vertical-align:middle;';
    const progressBarFill = document.createElement('div');
    progressBarFill.style.cssText = 'width:0%; height:100%; background:#4caf50; transition:width 0.2s;';
    ...
    async function startFetchingProcess(e, overrideParams = null) {
        ...
        try {
            for (let p = 1; p <= maxPage; p++) {
                ...
                progressBarFill.style.width = `${(progress * 100).toFixed(0)}%`;
            ...
            // Change mouse pointer to busy cursor when rendering begins
            document.body.classList.add('mb-sorting-active');

            // Wrap rendering logic in requestAnimationFrame to ensure calculations represent actual visual updates
            requestAnimationFrame(() => {
                ...
                // Use a nested requestAnimationFrame to measure when the browser actually finishes the painting
                requestAnimationFrame(() => {
