...
//     {version: '3.2.0+2026-02-23', description: 'Settings modal ghost-row fix (comprehensive): root cause identified as page-level CSS from MusicBrainz setting background:white (or similar) on generic <table>/<tr> elements, combined with hidden config-rows retaining phantom table height in some browser/CSS-cascade combinations — causing the white table background to bleed through the table\'s phantom height, growing ~1 unit per collapsed section. Fix applies four layers: (1) table and tbody get explicit background:#f9f9fb matching the scroll-wrapper div, so any phantom area is invisible. (2) Section-header <td> gets border:none inline to prevent any page CSS adding borders. (3) A scoped <style> block (prefixed with #scriptId-config-container) with !important rules forces table/tbody background to #f9f9fb, border-spacing to 0, explicitly sets display:none !important + height:0 + visibility:hidden on hidden config/subgrid rows, strips border-top/bottom from section-header tds, and keeps thead backgrounds opaque. (4) border-bottom removed from section-header <tr> inline style (was rgba(255,255,255,0.2) on <tr>, now absent on both <tr> and <td>).'},
        ...
        const settingsInterface = {
            ...
            showModal: function () {
                ...
                container.innerHTML = `
                    <style>
                        /*
                         * Hard-reset the settings modal table against MusicBrainz page-level CSS.
                         * Using !important so these rules win over any external stylesheet rules
                         * that might set background, border-spacing, or row height on generic
                         * table/tr/td selectors.
                         *
                         * Key problem: if the page CSS sets background:white on <table> or <tr>,
                         * that white shows through when config rows are display:none and the table
                         * retains phantom height — producing a white ghost strip that grows by
                         * ~1 unit per collapsed section.
                         *
                         * Fix: force the table/tbody/tr background to match the scroll-wrapper (#f9f9fb)
                         * so any phantom area is invisible. Also zero out all border-spacing.
                         */
                        #${scriptId}-config-container table {
                            background: #f9f9fb !important;
                            border-spacing: 0 !important;
                        }
                        #${scriptId}-config-container tbody {
                            background: #f9f9fb !important;
                        }
                        /* All non-section-header rows that are hidden */
                        #${scriptId}-config-container tr.config-row[style*="display: none"],
                        #${scriptId}-config-container tr.config-row[style*="display:none"],
                        #${scriptId}-config-container tr.mb-pd-subgrid[style*="display: none"],
                        #${scriptId}-config-container tr.mb-pd-subgrid[style*="display:none"] {
                        ...
                        /* Section headers: no border that could add phantom pixels */
                        #${scriptId}-config-container tr.section-header > td {
                            border-top: none !important;
                            border-bottom: none !important;
                        }
                        /* Thead background must be opaque to cover table background on sticky scroll */
                        #${scriptId}-config-container thead {
                            background: #e8ebef !important;
                        }
                        #${scriptId}-config-container thead tr {
                            background: #e8ebef !important;
                        }
                        #${scriptId}-config-container thead th {
                            background: #e8ebef !important;
                        }
                    </style>
