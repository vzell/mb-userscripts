// ==UserScript==
// @name         VZ: MusicBrainz - Auto-Select External Link Types For Events
// @namespace    https://github.com/vzell/mb-userscripts
// @version      1.4+2025-12-05
// @description  Auto-Select External Link Types on event pages, allows configuration of link mappings (URL Regex -> Link Type ID)
// @author       Gemini with vzell
// @tag          AI generated
// @homepageURL  https://github.com/vzell/mb-userscripts
// @downloadURL  https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypeEvents.user.js
// @updateURL    https://raw.githubusercontent.com/vzell/mb-userscripts/master/AutoSelectExternalLinkTypeEvents.user.js
...
(function() {
    ...
    const LOG_PREFIX = '[VZ:discography-autoselect-events]';

    function log(...args) {
        if (DEBUG) console.log(LOG_PREFIX, ...args);
    }

    const STORAGE_KEY_EVENTS = 'MB_AutoSelect_Mappings_Events';

    const LINK_TYPES_OPTIONS_EVENTS = [
    ...
    function loadMappings() {
        try {
            ...
            if (stored) {
                return JSON.parse(stored) || DEFAULT_MAPPINGS_EVENTS;
            }
        } catch (e) {
            log("Failed to load mappings from localStorage. Using defaults.", e);
        }
        return DEFAULT_MAPPINGS_EVENTS;
    }

    function saveMappings(mappings) {
        try {
            localStorage.setItem(STORAGE_KEY_EVENTS, JSON.stringify(mappings));
                ...
                mutation.addedNodes.forEach(node => {
                    ...
                    if (selectElement && lastCleanedUrlData) {
                        ...
                        setTimeout(() => {
                            ...
                            if (linkValueToSet) {
                                const selectedOption = LINK_TYPES_OPTIONS_EVENTS.find(o => o.id === linkValueToSet);
                                if (selectedOption) {
    ...
    function renderMappings(modalBody, openEditModalCallback) {
        ...
        linkMappings.forEach((mapping, index) => {
            const option = LINK_TYPES_OPTIONS_EVENTS.find(opt => opt.id === mapping.typeId);
    ...
    function openEditModal(index, reEnableMainEscape) {
        ...
        const currentMapping = isEditing ? linkMappings[index] : { regex: '', typeId: LINK_TYPES_OPTIONS_EVENTS[0].id, description: '' };
        ...
        modalContainer.innerHTML = `
            ...
            <div style="margin-bottom: 20px;">
                ...
                <select id="typeSelect" style="padding: 5px; border: 1px solid #ccc; width: 100%;">
                    ${LINK_TYPES_OPTIONS_EVENTS.map(opt =>
